"""
Compare official OLMo3-LHSA vs sglang HSA under REAL model config.

Config: 4096 hidden, 32 heads, 32 kv heads, hsa_heads=32, hsa_qk_ratio=1
  → ALL heads are HSA (no SWA upper branch)
  → enable_lmk_q_proj=True, unified_retrieval=True, retrieval_dim=1024

Tests prefill + decode alignment with patched eager (simulating FA2).

Usage:
  CUDA_VISIBLE_DEVICES=0 python dev/compare_real_config.py [--layers N]
"""
import os, sys, types, logging, tempfile, time, argparse

# ============================================================
# 1. Veomni mock (same as compare_olmo_lhsa_vs_sglang.py)
# ============================================================
...
# ============================================================
# 2. Import official model + patches
# ============================================================
...
# Patch eager attention to enforce sliding_window (simulating FA2)
...
# Patch tilelang kernels
import ops.topk_group as _tm, ops.hsa_fwd_bwd_group as _hm
_orig_topk = _tm.online_topk_group; _orig_hsa = _hm.HSA_block_M_group
def _patched_topk(q, k, topk, block_size, window_size, memory_window_size=-1, is_causal=True, **kw):
    return _orig_topk(q.contiguous(), k.contiguous(), topk, block_size=block_size,
                       window_size=window_size, memory_window_size=memory_window_size, is_causal=is_causal)
def _patched_hsa(q, k, v, weights, indices, block_size, mask_last_token=True, **kw):
    return _orig_hsa(q.contiguous(), k.contiguous(), v.contiguous(), weights=weights,
                      indices=indices, block_size=block_size, mask_last_token=mask_last_token)
_tm.online_topk_group = _patched_topk; _hm.HSA_block_M_group = _patched_hsa
...
def init_sglang():
    if not dist.is_initialized():
        _, p = tempfile.mkstemp(prefix='real_cmp_', suffix='.t')
...
def sglang_prefill_and_decode(sg_model, cfg, real_base, decode_tokens, device, dtype):
    ...
    prefill_logits = (h_normed @ sg_model.lm_head.weight[:VS, :].t()).float()
    prefill_logits = prefill_logits.unsqueeze(0)
...
def main():
    ...
    parser.add_argument('--layers', type=int, default=2, help='Number of layers (default 2 for quick test)')
    ...
    NL = args.layers
    PS = 64; VS = 100278
    HIDDEN = 4096; INTER = 11008
    NH = 32; NKV = 32; HD = HIDDEN // NH  # 128
    HSA_HEADS = 32; HSA_QK_RATIO = 1; HSA_TOPK = 16
    SW = 512

    print(f'Config: {NL} layers, hidden={HIDDEN}, heads={NH}, kv_heads={NKV}, head_dim={HD}')
    ...
    # Official config
    oc = OfficialConfig(
        vocab_size=VS, hidden_size=HIDDEN, intermediate_size=INTER,
        num_hidden_layers=NL, num_attention_heads=NH, num_key_value_heads=NKV,
        head_dim=HD, rms_norm_eps=1e-6, chunk_size=PS, hsa_topk=HSA_TOPK,
        hsa_mode='sparse', full_attn_interleave=4, hsa_heads=HSA_HEADS,
    ...
    print('Building official model...', flush=True)
    om = OfficialModel(oc).to(device=device, dtype=dtype); om.eval()
    print(f'  Official params: {sum(p.numel() for p in om.parameters()) / 1e6:.1f}M')
    ...
    print(f'  sglang params: {sum(p.numel() for p in sm.parameters()) / 1e6:.1f}M')

    # Fix sgl_kernel.rmsnorm CUDA bug: force native path
    ...
    # Quick sanity: compare embeddings and first-layer projections
    test_ids = torch.tensor([5, 10, 100, 1000, 50000], device=device)
    oemb = om.model.embed_tokens(test_ids)
    semb = sm.model.embed_tokens(test_ids)
    print(f'Embedding diff: {(oemb.float() - semb.float()).abs().max().item():.6f}')
    ...
    max_wdiff = 0
    for k in ssd:
        if k in osd and osd[k].shape == ssd[k].shape:
            d = (osd[k].float() - ssd[k].float()).abs().max().item()
            if d > 0.001:
                print(f'  Weight diff {k}: {d:.6f}')
            max_wdiff = max(max_wdiff, d)
    print(f'Max weight diff (direct keys): {max_wdiff:.6f}')

    print()
    print('=' * 70)
    print('REAL CONFIG: Official (FA2-simulated) vs sglang Alignment')
    print('=' * 70)

    decode_toks_short = [42, 100, 200, 50, 150, 75, 30, 180]
    decode_toks_long = [42, 100, 200, 50, 150]
    test_cases = [
        (10, decode_toks_short, 'SHORT: 10 tokens'),
        (63, decode_toks_short, 'SHORT: 63 tokens (1 page - 1)'),
        ...
        (2048, decode_toks_long, 'LONG: 2048 tokens (32 pages)'),
    ]
    ...
    for n_prefill, decode_toks, desc in test_cases:
        ...
        eng_len = len(fi_base)
        off_prefill = off_logits[0, :eng_len]
        sg_prefill = sg_prefill_logits[0, :eng_len]
