...
We benchmark the end-to-end inference latency of HiLS-Attention against full attention in the SGLang inference engine on a single NVIDIA H200 GPU. % TODO: \citep{zheng2024sglang}
To ensure an apples-to-apples comparison, both backends share the same Triton attention-kernel infrastructure and differ only in the attention algorithm; in particular the full-attention baseline is run with the same Triton kernels rather than a vendor-optimized paged-attention implementation. We use the 345M-scale architecture of \S\ref{sec:small_scale_exp} with chunk size 64, top-$k=32$, and a 512-token sliding window, set the paging granularity of the KV cache equal to the chunk size, and decode in a single stream (batch size 1) in bf16. We report warm-run prefill latency for the full input and the median per-token decode latency over 32 steps with CUDA graphs enabled.

\input{table/Efficiency}

As shown in Table~\ref{tab:efficiency}, the cost of HiLS-Attention is governed by its fixed retrieval budget ($K\times\text{chunk}=2048$ tokens) plus the local sliding window, rather than by the full context length. Consequently its prefill latency grows \emph{near-linearly} with context length while full attention grows quadratically, and its per-token decode latency stays \emph{effectively constant} ($\mathcal{O}(1)$) while full attention grows linearly with the KV-cache length. The two methods cross over at roughly 16K tokens: below this length the fixed cost of the selector and the dual sliding-window/retrieval branches is not yet amortized, so full attention is faster; at and beyond 16K, HiLS-Attention reaches parity on prefill and is already faster on decode. Past the crossover the gap widens rapidly with length---at 512K context HiLS-Attention is $25.7\times$ faster in prefill (2.6\,s vs.\ 66.2\,s) and $15.0\times$ faster per decode step (3.3\,ms vs.\ 49.1\,ms)---making HiLS-Attention substantially more practical than full attention for long-context serving while matching its quality (\S\ref{sec:small_scale_exp}).
