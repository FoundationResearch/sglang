...
def fused_topk_forward_kernel_insert(
    ...
    if BLOCK_S is None:
        BLOCK_S = 16
    # 对 head_dim 进行切分，避免 shared memory 爆炸
    BLOCK_HD = min(head_dim, 128)
    num_d_blocks = tilelang.cdiv(head_dim, BLOCK_HD)
    if threads is None:
        threads = BLOCK_L
    ...
    def fwd_kernel_insert(
        ...
        with T.Kernel(tilelang.cdiv(seq_len, BLOCK_L), heads, batch, threads=threads) as (bx, by, bz):
            ...
            Q_shared = T.alloc_shared([BLOCK_L, BLOCK_HD], dtype)
            K_shared = T.alloc_shared([BLOCK_S, BLOCK_HD], dtype)
            ...
            for s_block in T.serial(loop_limit):
                ...
                T.clear(acc_s)
                # 沿 head_dim 维度分块累加：Q[L,HD] × K[S,HD]^T，逐块求和
                for d_block in T.serial(num_d_blocks):
                    base_d = d_block * BLOCK_HD

                    for l_idx, d in T.Parallel(BLOCK_L, BLOCK_HD):
                        tq = base_l + l_idx
                        dd = base_d + d
                        if (tq < seq_len) and (dd < head_dim):
                            Q_shared[l_idx, d] = Q[b, tq, h, dd]
                        else:
                            Q_shared[l_idx, d] = T.Cast(dtype, 0.0)

                    for s_idx, d in T.Parallel(BLOCK_S, BLOCK_HD):
                        ts = base_s + s_idx
                        dd = base_d + d
                        if (ts < s_len) and (dd < head_dim):
                            K_shared[s_idx, d] = K[b, ts, h, dd]
                        else:
...
class OnlineTopKFusedFn(torch.autograd.Function):
    ...
    def forward(ctx, q, lmks, topk: int, fwd_kernel, sort_kernel, q_offset_tensor, drop_mask=None):
        ...
        #     drop_mask = torch.zeros(B, L, S, device=q.device, dtype=torch.int32)
...
class OnlineTopK_Fused(torch.nn.Module):
    ...
    def forward(self, q, lmks, q_offset, drop_mask=None):
        ...
        fwd_kernel = fused_topk_forward_kernel_insert(
            B, h_kv, D, topk, block_size, window_size, is_causal,
...
def test_online_topk_fused_correctness():
    ...
    # 修改 1: 传入 q_offset
    indices_fused, scores_fused = online_topk_group(q, lmks, topk, block_size, window_size, is_causal, memory_window_size=memory_window_size, q_offset=q_offset, is_training=False)
    ...
    indices_fused_bwd, scores_fused_bwd = online_topk_group(q_fused, lmks_fused, topk, block_size, window_size, is_causal, memory_window_size=memory_window_size, q_offset=q_offset, is_training=False)
