...
def _attn_fwd_inner(acc, l_i, m_i, q,  #
    ...
    # m_ij = tl.max(qk, 1) * qk_scale  # (group_size * M, N)
    # qk = tl.clamp(qk, min=-127 / qk_scale, max=128 / qk_scale)
    m_ij = tl.maximum(m_i, tl.max(qk, 1) * qk_scale)
    qk = qk * qk_scale - m_ij[:, None]  # (group_size, M, N) 
    # if _has_nan(qk):
    # #     tl.device_print('qk vals: ', qk)
    # if _has_nan(m_ij):
    #     tl.device_print('m_ij : ', m_ij, qk)
    p = tl.exp2(qk)  # （group_size * M, N)
    l_ij = tl.sum(p, 1)  # axis=1 （group_size * M）
    # -- update m_i and l_i
    # if _has_nan(l_ij):
    #     tl.device_print('after tl.sum, l_ij : ', l_ij)

    alpha = tl.exp2(-m_ij)  # (group_size * M) 
    # if _has_nan(alpha):
    #     tl.device_print('alpha', alpha, -m_ij)
    l_i = l_i * alpha + l_ij
    # if _has_nan(l_i):
    #     tl.device_print('scaled l_i : ', l_i)
    #     tl.device_print('alpha: ', alpha)
    #     tl.device_print('l_ij : ', l_ij)
    #     tl.device_print('sum p: ', tl.max(p))
...
def kernel_attn_bwd_dq(Q, K, V, weights,  #
    ...
    for chunk_i in range(TOPK):
        ...
        # tl.device_assert(~tl.min(tl.sum(p, 1) <= 1), msg='p sum > 1')
        # if tl.max(tl.sum(p, 1)) > 1.1:
        #     tl.device_print('p sum error', tl.sum(p, 1))
            # tl.device_print('p sum error mi: ', mi)
            # tl.device_print('p sum error qk: ', qk)
        o = tl.dot(p, vi.to(tl.float32))  # (G * block_m, dim)
        Di = tl.sum(o * do, 1)  # (G * block_m)
        # tl.device_assert(~_has_nan(Di))
        # if _has_nan(Di):
        #     tl.device_print('Di has nan', Di)
        ...
        # l1 reg
        # qk_max = tl.max(tl.abs(qk), axis=1)  # (G * block_m)
        # mask = qk_max > reg_C  # (G * block_m)
        # mask = tl.expand_dims(mask, 1)  # (G * block_m, 1)
        # mask = tl.broadcast_to(mask, qk.shape)
        # sign = tl.where(qk > 0, 1.0, -1.0) # (G * block_m, block_kv)
        # qk_sign = tl.where(mask, sign, 0.0)
        # dq += reg_lamda * tl.dot(qk_sign, tl.trans(ki).to(tl.float32)) # (G * block_m, dim)

        # l2_reg
        # dq += (2 * reg_lamda) * tl.dot(qk, tl.trans(ki).to(tl.float32)) / (reg_C * reg_C)
    ...
    for block_m in range(m_start, m_blocks):
        ...
        if m_i:
            ...
            if K_idx >= 0:
                ...
                # l1 regularization
                # qkT_max = tl.max(tl.abs(qkT), axis=0)
                # mask = tl.abs(qkT_max) > reg_C  # (BLOCK_M * GROUP_NUM)
                # mask = tl.expand_dims(mask, 0)  # (1, G * block_m)
                # mask = tl.broadcast_to(mask, qkT.shape)  # (BLOCK_KV, BLOCK_M * GROUP_NUM)
                # sign = tl.where(qkT > 0, 1.0, -1.0) # (BLOCK_KV, BLOCK_M * GROUP_NUM)
                # qk_sign = tl.where(mask, sign, 0.0) # (BLOCK_KV, BLOCK_M * GROUP_NUM)
                # dk += reg_lamda * tl.dot(qk_sign, tl.trans(qT).to(tl.float32)) # (BLOCK_KV, dim)

                # l2_reg
                # dk += (2 * reg_lamda) * tl.dot(qkT, tl.trans(qT).to(tl.float32)) / (reg_C * reg_C)
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, weights, indices, sm_n, chunk_size, sm_scale, l2_lamda, l2_C):
        ...
        ctx.l2_C = l2_C
        # print(f'M min max: {M.min()}, {M.max()}')
        # if torch.any(torch.isnan(M)) or torch.any(torch.isnan(o)):
        #     torch.save(M, 'M.tensor')
        #     torch.save(o, 'o.tensor')
        #     torch.save(q, 'q.tensor')
        #     torch.save(k, 'k.tensor')
        #     torch.save(v, 'v.tensor')
        #     # torch.save(Li, 'Li.tensor')
        #     torch.save(weights, 'w.tensor')
        #     torch.save(indices, 'ind.tensor')
        #     assert False, f'M has NAN? : {torch.any(torch.isnan(M))}, o has NAN? : {torch.any(torch.isnan(o))}'
        # o.nan_to_num_(nan=0.0)
    ...
    def backward(ctx, do):
        ...
        #     offset = L * local_rank
        #     print(f'cp bwd HSA weights: {weights[0, : 5, 0, :]} q {q[0, : 5, 0, :5]} k {k[0, offset: offset + 5, 0, :5]} v {v[0, offset: offset + 5, 0, :5]}')
        # else:
        #     local_rank = torch.distributed.get_rank()
        #     world_size = torch.distributed.get_world_size()
        #     offset = L // world_size * local_rank
        #     print(f'non-cp bwd HSA weights: {weights[0, offset: offset + 5, 0, :]} q {q[0, offset: offset + 5, 0, :5]} k {k[0, offset: offset + 5, 0, :5]} v {v[0, offset: offset + 5, 0, :5]}')
        ...
        # print(f'block mask: {block_mask[0, :, 0, :5].transpose(0, 1)}')
        # print(f'rev idx: {rev_idx[0, :, 0, :5].transpose(0, 1)}')
        # print(f'rev idx: {rev_idx[0, 0, 0, :]}')
        # print(f'block mask sum: {block_mask[0, 0, 0, :].sum()}')
        ...
        # print(f'block mask: {block_mask}, sum: {block_mask.sum(dim=-1)}')
        # tmp_rev_idx = rev_idx * block_mask
        # print(f'rev_idx : {rev_idx}, rev_idx hash: {tmp_rev_idx.sum(dim=-1)}')
        ...
        #     world_size = torch.distributed.get_world_size()
        #     offset = L * local_rank
        #     print(f'cp bwd HSA dq: {dq[0, :5, 0, :5]} dw: {dw[0, -5:, 0, :5]} dk: {dk[0, :5, 0, :1]}')
        # else:
        #     local_rank = torch.distributed.get_rank()
        #     world_size = torch.distributed.get_world_size()
        #     offset = L // world_size * local_rank
        #     print(f'non-cp bwd HSA dq: {dq[0, offset: offset + 5, 0, :5]}, dw: {dw[0, -5:, 0, :5]}, dq: {dq[0, :5, 0, :5]} dk: {dk[0, :5, 0, :5]} D:{D[0, :, -5:, 0]}')
...
# @pytest.mark.parametrize("Z, HQ, H, N_CTX, HEAD_DIM, K, S, Q_X", [(2, 16, 1, 256, 64, 4, 64, 1), (2, 16, 4, 256, 64, 4, 64, 16), (1, 16, 1, 2048, 16, 4, 32, 1)])
@pytest.mark.parametrize("Z, HQ, H, N_CTX, HEAD_DIM, K, S, Q_X", [(1, 16, 1, 32, 16, 2, 16, 1)])#, (2, 32, 2, 1024, 16, 4, 32, 1), (2, 32, 2, 1324, 16, 4, 32, 1), (2, 32, 2, 1024, 32, 4, 64, 1), (2, 32, 2, 1324, 32, 4, 64, 1)])
def test_group_qa(Z, HQ, H, N_CTX, HEAD_DIM, K, S, Q_X, dtype=torch.float32):
    import torch.nn.functional as F
    import os
    ...
    q = (torch.empty((Z, N_CTX, HQ, HEAD_DIM), dtype=dtype, device="cuda").normal_(mean=0.0, std=0.05).requires_grad_())
    k = (torch.empty((Z, N_CTX // S * S, H, HEAD_DIM), dtype=dtype, device="cuda").normal_(mean=0.0, std=0.05).requires_grad_())
    v = (torch.empty((Z, N_CTX // S * S, H, HEAD_DIM), dtype=dtype, device="cuda").normal_(mean=0.0, std=0.05).requires_grad_())
    # q = (torch.empty((Z, N_CTX, HQ, HEAD_DIM), dtype=dtype, device="cuda").fill_(0.1).requires_grad_())
    # k = (torch.empty((Z, N_CTX // S * S, H, HEAD_DIM), dtype=dtype, device="cuda").fill_(0.1).requires_grad_())
    # v = (torch.empty((Z, N_CTX // S * S, H, HEAD_DIM), dtype=dtype, device="cuda").fill_(0.1).requires_grad_())

    # print(f'v shape: {v.shape}')
    chunk_num = N_CTX // S
    q_blocks = N_CTX // Q_X
    RCP_LN2 = 1.4426950408889634
    # indices = torch.randint(0, chunk_num, size=(Z, q_blocks, H, K), device='cuda')
    indices = torch.arange(0, K).unsqueeze(0).unsqueeze(0).repeat(Z, q_blocks, 1)

    for i in range(Z):
        for j in range(q_blocks):
            max_visible_k_block = min(j * Q_X // S, K)
            if max_visible_k_block > 0:
                indices[i, j, max_visible_k_block - 1] = 0
                indices[i, j, 0] = max_visible_k_block - 1
    # print(indices)
    indices = indices.unsqueeze(2).expand(-1, -1, H, -1)
    print(f'indices: {indices}')
    
    scores = torch.tensor(torch.rand(Z, q_blocks, H, K), dtype=dtype, device='cuda')
    # scores = torch.tensor(torch.zeros(Z, q_blocks, H, K), dtype=dtype, device='cuda')
    score_mask = torch.zeros(Z, q_blocks, H, K, dtype=torch.bool)
    for i in range(Z):
        for j in range(q_blocks):
            for h in range(H):
                for _k in range(K):
                    q_offset = j * Q_X
                    k_end = indices[i, j, h, _k] * S + S
                    score_mask[i, j, h, _k] = q_offset >= k_end
    # score_mask = score_mask.expand(-1, -1, H, -1)
    scores.masked_fill_(~score_mask.cuda(), float('-inf'))

    # (N, q_blocks, K)
    weights = F.softmax(scores.to(torch.float32), dim=-1).nan_to_num_(0.0).requires_grad_()
    print(f'weights: {weights}')

    # weights = torch.ones((Z, q_blocks, K), dtype=dtype, device='cuda').requires_grad_()
    ...
    chunked_k = rearrange(k, 'Z (N S) h d->Z N S h d', S=S)
    chunked_v = rearrange(v, 'Z (N S) h d->Z N S h d', S=S)
    h = chunked_k.shape[-2]
    d = chunked_k.shape[-1]

    indices = indices.cuda()
    indices_ = rearrange(indices, 'Z B h K->Z (B K) h')
    indices_ = indices_.unsqueeze(2).unsqueeze(-1)
    indices_expanded = indices_.expand(-1, -1, S, -1, d) 
    gather_k = chunked_k.gather(dim=1, index=indices_expanded)
    gather_v = chunked_v.gather(dim=1, index=indices_expanded)
    # print(f'gather v shape: {gather_v.shape}')

    gather_k = rearrange(gather_k, 'Z (B K) S h d->Z B S K h d', K=K)
    gather_v = rearrange(gather_v, 'Z (B K) S h d->Z B S K h d', K=K)

    k_ = torch.repeat_interleave(gather_k, dim=-2, repeats=HQ // H)  # (Z B K S 4*h d)
    v_ = torch.repeat_interleave(gather_v, dim=-2, repeats=HQ // H)  # (Z B K S 4*h d)
    q_chunked = rearrange(q, 'Z (B X) h d->Z B X h d', X=Q_X)

    qk = torch.einsum('Z B X h d, Z B S K h d->Z B X S K h', q_chunked.float(), k_.float()) * sm_scale
    ref_M = qk.logsumexp(dim=-3) * 1.4426950408889634  # (Z B X K h)
    ref_M = rearrange(ref_M, 'Z B X K h->Z K (B X) h')
    def softmax_off_by_one(tensor, sm_n=0.0, dim=-1):
    ...
    p = softmax_off_by_one(qk, sm_n=1.0, dim=-3)  # Z B X S K h
    assert p.dtype == torch.float32
    o_k = torch.einsum('Z B X S K h, Z B S K h d->Z B X K h d', p, v_.float())
    print(f'o_k shape: {o_k.shape}')
    w_exp = torch.repeat_interleave(weights, dim=-2, repeats=HQ // H)
    
    ref_o = torch.einsum('Z B X K h d, Z B h K->Z B X h d', o_k, w_exp.float())
    ref_o = rearrange(ref_o, 'Z B X h d->Z (B X) h d')
    assert ref_o.dtype == torch.float32
    ...
    tri_o = HSA(q, k, v, weights, indices, 1.0, S, sm_scale, reg_lamda=0.0, reg_C=0.0)
    tri_o.backward(dout)
    ...
    print(f'tri o dtype: {tri_o.dtype}, ref_o dtype: {ref_o.dtype}')

    print(f'tri_o: {tri_o[0, -5:, 0, :5]}')
    print(f'ref_o: {ref_o[0, -5:, 0, :5]}')
    print(f'tri_dq: {tri_dq[0, :, 0, :5]}')
    print(f'ref_dq: {ref_dq[0, :, 0, :5]}')
    print(f'tri_dk: {tri_dk[0, -5:, 0, :5]}')
    print(f'ref_dk: {ref_dk[0, -5:, 0, :5]}')
    # print(f'weights: {weights[0, -5:, -1]}')
    # print(f'indices: {indices[0, -5:, :].sum(dim=-1)}')
    o_k = rearrange(o_k, 'Z B X K h d->Z (B X) K h d')
    print(f'o_k: {o_k[0, -5:, -1, 0, :5]}')
    # Z B S K h d
    print(f'gather_v: {gather_v[0, -5:, :, -1, 0, :5].sum(dim=1)}')

    assert_close('o', tri_o, ref_o, 0.005)
    assert_close('dw', tri_dw, ref_dw, 0.005)
    assert_close('dq', tri_dq, ref_dq, 0.005)
    assert_close('dk', tri_dk, ref_dk, 0.005)
    assert_close('dv', tri_dv, ref_dv, 0.005)

    # tri_o = HSA(q.to(torch.float32), k.to(torch.float32), v.to(torch.float32), weights, indices, 1.0, S, sm_scale, reg_lamda=0.1, reg_C=0.0)
    # tri_o.backward(dout)
    # tri_dv, v.grad = v.grad.clone(), None
    # tri_dk, k.grad = k.grad.clone(), None
    # tri_dq, q.grad = q.grad.clone(), None
    ...
    # print(get_err_ratio(tri_dq, ref_dq))
    # print(get_err_ratio(tri_dk, ref_dk))
    # # print(f'{tri_dk.shape} vs {ref_dk.shape}')
    # assert False
    # assert_close('o', tri_o, ref_o, 0.5)
    # assert_close('dq', tri_dq, ref_dq, 0.5)
    # assert_close('dk', tri_dk, ref_dk, 0.5)
    # assert_close('dv', tri_dv, ref_dv, 0.5)
    # assert_close('dw', tri_dw, ref_dw, 0.5)


# @pytest.mark.parametrize("Z, HQ, H, N_CTX, HEAD_DIM, K, S, Q_X, Q_LEN", [(1, 32, 2, 1024, 16, 4, 32, 1, 4)])
# def test_qa_inference(Z, HQ, H, N_CTX, HEAD_DIM, K, S, Q_X, Q_LEN, dtype=torch.bfloat16):
#     import torch.nn.functional as F
#     torch.manual_seed(22)
#     # q = (torch.empty((Z + 1, H * (N_CTX + 1), HEAD_DIM), dtype=dtype, device="cuda").normal_(mean=0.0, std=0.5).requires_grad_())
#     # q = rearrange(q, 'Z (H L) D -> Z H L D ', H=H)
...
#     # q_ = q.contiguous()
#     # torch.set_printoptions(threshold=float('inf'))
#     q = (torch.empty((Z, Q_LEN, HQ, HEAD_DIM), dtype=dtype, device="cuda").normal_(mean=0.0, std=0.5).requires_grad_())
#     k = (torch.empty((Z, N_CTX, H, HEAD_DIM), dtype=dtype, device="cuda").normal_(mean=0.0, std=0.5).requires_grad_())
#     v = (torch.empty((Z, N_CTX, H, HEAD_DIM), dtype=dtype, device="cuda").normal_(mean=0.0, std=0.5).requires_grad_())
#     # print(f'v shape: {v.shape}')
#     chunk_num = N_CTX // S
#     q_blocks = Q_LEN // Q_X
#     RCP_LN2 = 1.4426950408889634
...
#     scores = torch.tensor(torch.rand(Z, q_blocks, H, K), dtype=dtype, device='cuda')
#     # score_mask = torch.zeros(Z, q_blocks, K, dtype=torch.bool)
#     # for i in range(Z):
#     #     for j in range(q_blocks):
#     #         for _k in range(K):
#     #             q_offset = j * Q_X
#     #             k_end = indices[i, j, _k] * S + S
#     #             score_mask[i,j,_k] = q_offset >= k_end
    
#     # scores.masked_fill_(~score_mask.cuda(), float('-inf'))

#     # (N, q_blocks, K)
#     weights = F.softmax(scores, dim=-1).nan_to_num_(0.0).requires_grad_()
...
#     print(f'gather_k shape: {gather_k.shape}')

#     k_ = torch.repeat_interleave(gather_k, dim=-2, repeats=HQ // H)  # (Z B K S 4*h d)
#     v_ = torch.repeat_interleave(gather_v, dim=-2, repeats=HQ // H)  # (Z B K S 4*h d)
#     q_chunked = rearrange(q, 'Z (B X) h d->Z B X h d', X=Q_X)

#     print(f'q_chunked shape: {q_chunked.shape}, k shape: {k_.shape}')
#     qk = torch.einsum('Z B X h d, Z B S K h d->Z B X S K h', q_chunked.float(), k_.float()) * sm_scale
#     ref_M = qk.logsumexp(dim=-3) * 1.4426950408889634  # (Z B X K h)
#     ref_M = rearrange(ref_M, 'Z B X K h->Z K (B X) h')
#     def softmax_off_by_one(tensor, sm_n=0.0, dim=-1):
...
#     tri_o = HSA(q, k, v, weights, indices, 1.0, S, sm_scale, reg_lamda=0.00)
#     # print(tri_o)
