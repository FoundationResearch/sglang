...
def rotate_half_torch(x):
    """Rotates half the hidden dims of the input. 用于测试的 PyTorch 参考实现。"""
    x1 = x[..., : x.shape[-1] // 2]
    x2 = x[..., x.shape[-1] // 2 :]
...
def test_hsa_rope_correctness():
    """
    验证 hsa_intra_chunk_rope kernel（训练专用合并版，L_q == L_k）
    与 PyTorch 参考实现的数值一致性。
    参考实现完整复现 lhsa_layer.py 中 apply_hsa_rope 的计算路径。
    """
    ...
    # ---- 配置参数 ----
    B = 2
    L = 512*16          # Q 和 K 的共同序列长度（训练时 L_q == L_k）
    H_q = 16          # Q head 数
    H_k = 4          # K head 数（GQA）
    D = 64          # head dim
    chunk_size = 64
    hsa_sliding_window = 0
    max_seq_len = 2 * chunk_size + hsa_sliding_window + 256  # cos/sin 表的最大长度
    ...
    # 全局 cos/sin 表 (1, max_seq_len, D)，所有 batch 共享（与真实 Qwen3RotaryEmbedding 行为一致）
    # 真实 RoPE 结构：emb = cat((freqs, freqs), dim=-1)，前后半相同
    ...
    # ================================================================
    # 参考实现（PyTorch，完整复现 lhsa_layer.py 的计算路径）
    # ================================================================
    ...
    print("✅ HSA Intra-Chunk RoPE kernel（合并版）正确性验证通过！")
    
    # ================================================================
    # 额外测试：fixed position 模式（chunk_size_q=1）
    # ================================================================
    print("\n--- 测试 Fixed Q Position 模式 (chunk_size_q=1) ---")
    ...
    print("✅ HSA Intra-Chunk RoPE 反向传播梯度验证通过！")
...
def test_single_tensor_rope_performance():
    """
    Single Tensor RoPE 性能测试。
    对比 PyTorch 原始路径 vs TileLang Kernel 的延迟。
    """
    print("\n" + "=" * 60)
    ...
    # cos/sin: (1, L, D)，所有 batch 共享（与真实 Qwen3RotaryEmbedding 行为一致）
    freqs = torch.randn((1, L, D // 2), dtype=torch.float32, device=device)
    emb = torch.cat((freqs, freqs), dim=-1)  # (1, L, D)
    ...
    # ------------------------------------------------------------------
    print("-" * 50)
    t_baseline = benchmark(pytorch_baseline, "PyTorch 原始路径")
    t_kernel = benchmark(tilelang_kernel_only, "TileLang 纯 kernel")
    
    print("-" * 50)
    print(f"  加速比: {t_baseline / t_kernel:.2f}x")
    print("=" * 60)
...
def test_hsa_rope_performance():
    """
    HSA Intra-Chunk RoPE 性能测试。
    对比两条路径的端到端延迟：
      1. PyTorch 原始路径（构造 cos/sin + apply）
      2. TileLang Kernel 路径（slice + kernel）
    """
    print("\n" + "=" * 60)
    ...
    # 真实 RoPE 结构：emb = cat((freqs, freqs), dim=-1)，前后半相同
    # 所有 batch 共享同一组 cos/sin（与真实 Qwen3RotaryEmbedding 行为一致）
    freqs = torch.randn((1, max_seq_len, D // 2), dtype=torch.float32, device=device)
    ...
    # ------------------------------------------------------------------
    # 路径 1: PyTorch 原始路径（构造 cos/sin + apply，完整复现 lhsa_layer.py）
    # ------------------------------------------------------------------
    def pytorch_baseline():
        L_full = hsa_k_norm.shape[1]
        
        # 构造 Q 的 cos/sin（repeat）
    ...
    # ------------------------------------------------------------------
    # 路径 2: TileLang Kernel 路径（slice + kernel）
    # ------------------------------------------------------------------
    def tilelang_kernel():
        # 外部 slice + contiguous
    ...
    # ------------------------------------------------------------------
    # 路径 3: TileLang Kernel 路径（预构造 slice，纯 kernel 调用）
    # ------------------------------------------------------------------
    def tilelang_kernel_only():
        # 直接用预构造好的 slice，只测 kernel 本身
    ...
    # ------------------------------------------------------------------
    print("-" * 50)
    t_baseline = benchmark(pytorch_baseline, "PyTorch 原始路径 (构造+apply)")
    t_kernel_e2e = benchmark(tilelang_kernel, "TileLang 端到端 (slice+kernel)")
    t_kernel_only = benchmark(tilelang_kernel_only, "TileLang 纯 kernel")
    
    print("-" * 50)
    print(f"  加速比 (原始 vs 端到端): {t_baseline / t_kernel_e2e:.2f}x")
    print(f"  加速比 (原始 vs 纯kernel): {t_baseline / t_kernel_only:.2f}x")
    print("=" * 60)


if __name__ == "__main__":
    # 运行 Single Tensor RoPE 正确性测试
    test_single_tensor_rope_correctness()
    
    # 运行 Single Tensor RoPE 性能测试
