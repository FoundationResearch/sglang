...
def ref_softmax_topk_max_pooling(q, k_lmks, lse_swa, topk, block_size, window_size, is_causal=False, q_offset=0, drop_mask=None, bias=None, gumbel_noise=None):
    ...
    # DropMask 支持：将被 drop 的 chunk 的 max-pooling score 设为 -inf
    ...
    # ✅ 关键修复：将 score 为 -inf 的 indices 标记为 -1（无效）
    # kernel 的 is_valid 检查会跳过 causal mask / drop_mask 导致的无效 chunk，
    # 但 torch.topk 在所有值都是 -inf 时仍然会返回 indices。
    # 这里需要和 kernel 保持一致：无效 chunk 的 index 应该是 -1。
...
def hsa_lse_kernel(
    ...
    # K: [B, S, h_kv, D] (shared) or [B, S, h_kv*G, D] (per-q-head)
    if per_qhead_lmks:
    ...
    def kernel(
        ...
        with T.Kernel(tilelang.cdiv(seq_len_var, BLOCK_L), h_kv, batch, threads=threads) as (bx, by, bz):
            ...
            # Per-g GEMM staging when per_qhead_lmks=True
            ...
            for s_block in T.serial(loop_limit):
                ...
                if per_qhead_lmks:
                    # Per-q-head K: G separate small GEMMs.  Write each g's
                    # output into shared `score_shared` directly, since fragment
                    # buffers cannot be indexed with a non-unit-stride pattern.
                    ...
                    # [FIX 2] Copy Fragment -> Shared so subsequent masking can edit values
...
def recompute_topk_max_pooling_scores_kernel(
    ...
    def fwd_recompute(
        ...
        with T.Kernel(tilelang.cdiv(seq_len_var, BLOCK_L), h_kv, batch, threads=threads) as (bx, by, bz):
            ...
            if per_qhead_lmks:
                # Per-g GEMM staging; avoid the large [GEMM_M, GEMM_N] score_shared
                # to keep dynamic shared memory within device limits.
                ...
                for tk_block in T.serial(tk_blocks):
                    ...
                    for g in T.serial(groups):
                        ...
                        # Write directly to OutScores from fragment.
...
class SoftmaxTopKMaxPoolingFusedFn(torch.autograd.Function):
    ...
    def backward(ctx, grad_indices_unused, grad_scores_selected):
        ...
        if per_qhead_lmks:
            # k_in: [B, S, h_kv*G, D] -> view [B, S, h_kv, G, D] -> bmm-friendly layout
        ...
        if per_qhead_lmks:
            # Each (h_kv, g) has its own k -> no sum over G; output shape [B, S, h_kv*G, D]
...
def online_softmax_topk_head(
    ...
    对每个 query token，在其可见的 landmark key chunks 中，使用 softmax-then-max-pooling 策略
    选出 score 最高的 topk 个，返回排序后的 chunk indices 和对应 scores。
    不同的 q heads 虽然共享检索 chunks 但不共享检索 chunk 分数。
    支持 causal mask、sliding window、DropMask、GQA/MQA 自适应。
    支持 autograd 反向传播。
    ...
    Args:
        ...
        lse_swa (torch.Tensor):
            SWA 的 LSE。shape = [B, L, h_q] 或 [B, L, h_kv, G]
        topk (int):
        ...
        q_offset (int, default=0):
            query 在 KV cache 中的全局偏移量，用于 inference 时正确计算 causal 边界。
        is_training (bool, default=True):
            是否为训练模式。False 时 seq_len/s_len 使用 dynamic shape。
        drop_mask (torch.Tensor, optional):
            shape = [B, L, S]，dtype = int32，0/1 bitmap。
            1 表示该 chunk 被 drop，不参与 topk 选择。
    ...
    Returns:
        ...
        - indices: [B, L, h_kv, topk]，按升序排列（始终是 KV head 维度）
        - scores: [B, L, h_q, topk]，对应的 raw scaled qk scores
    """
...
def test_train_inference_correctness(test_name, B, q_len, kv_len, h_kv, G, D, topk, block_size, window_size, is_training, q_offset, use_bias, use_gumbel, per_qhead_lmks=False, is_causal=True):
    ...
    # Lmks: 默认 [B, S, h_kv, D]; per_qhead 模式下 [B, S, h_q, D]
    if per_qhead_lmks:
...
def test_gqa_d_reshape_correctness():
    ...
    # 计算全量 scores: [B, L, h_kv_new, G_new, S]
