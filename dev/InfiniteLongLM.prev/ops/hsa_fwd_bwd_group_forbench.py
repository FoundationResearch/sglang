        ...
        with T.Kernel(tilelang.cdiv(q_len, M), NV, batch * head_kv, threads=num_threads) as (bx, by, bz):
            ...
            for i in T.Pipelined(merged_len_local, num_stages=num_stages):
                ...
                if (blk_idx >= 0):
                    ...
                    # 6. Normalize & Apply Head-specific Weights
                    for r, c in T.Parallel(M_G, BS):
                        q_idx = r // groups
                        # is_valid_q = (ownership & (1 << q_idx)) != 0
                        # if is_valid_q:
                        #     # 获取当前 Head 的独立权重
                        #     weight = T.cast(chunk_weights[i, q_idx], accum_dtype)
                        #     acc_s[r, c] = acc_s[r, c] * weight / scores_sum[r]
                        # else:
                        #     acc_s[r, c] = 0.0
                        acc_s[r, c] = T.if_then_else(
                                                    (ownership & (1 << q_idx)) != 0,
                                                    acc_s[r, c] * T.cast(chunk_weights[i, q_idx], accum_dtype) / scores_sum[r],
                                                    0.0
                                                )

                    # 7. Store P to Shared Memory (float -> bfloat16)
                    T.copy(acc_s, P_shared)

                    # 8. Load V & Compute O (P * V)
                    T.copy(V[i_b, i_s:i_s + BS, i_h, i_v * BV:(i_v + 1) * BV], V_shared)
...
def hierarchical_sparse_attention_bwd_dqkv_block_M(
    ...
    if block_M is None or block_M <= 0:
        ...
        M = max(block_M, M_min)     
    print("Using M =", M, "for bwd_block_M kernel", "num_threads:", num_threads)
    M_G = M * G
        ...
        with T.Kernel(NV, NS_kv, B * heads_kv, threads=num_threads) as (i_v, i_s, i_bh):
            ...
            for ip in T.Pipelined(NP, num_stages=num_stages):
                ...
                if any_valid != 0:
                    ...
                    # === WORKAROUND for Codegen Bug (val[...] error) ===
                    # Instead of fused load-mask with T.if_then_else expression,
                    # we do: 1. Load, 2. Mask via Control Flow (if statement).
                    ...
                    for i, s in T.Parallel(M_G, BS):
                        ...
                        acc_s_tmp[i, s] = acc_s_tmp[i, s] * inv_sum
                    ...
                    for i in T.Parallel(M):
                        tq = base_t + i
                        if tq < q_len and has_q[i] == 1 and pos[i] != -1:
                            W_local[i] = W[i_b, tq, i_h, pos[i]]
                    ...
                    dw_row_sum_frag_2d = T.reshape(dw_row_sum_frag, [M, G])  
                    dw_row_sum_frag_M = T.alloc_fragment([M], accum_dtype)
                    T.reduce_sum(dw_row_sum_frag_2d, dw_row_sum_frag_M, dim=1, clear=True)  
                    for qi5 in T.Parallel(M):
                        if has_q[qi5] == 1:  
                            tq = base_t + qi5  
                            if tq < q_len and pos[qi5] != -1:  
                                DW[i_b, tq, i_h, pos[qi5]] = dw_row_sum_frag_M[qi5]

                    # [Step 4] 准备 dV 和 dS 的输入
                    # 更新 dO_shared 为 dO_weighted (原地更新)
                    for row_idx, v in T.Parallel(M_G, BV):
                        dO_weighted_shared[row_idx, v] = W_local[row_idx // G] * dO_shared[row_idx, v]
                    
                    # 更新 Z (dV_PdO_frag) 为 W * Z
                    for g_row, s in T.Parallel(M_G, BS):
                        dV_PdO_frag[g_row, s] = dV_PdO_frag[g_row, s] * W_local[g_row // G]
...
class _hsa_block_M_attention(torch.autograd.Function):
    ...
    def forward(
        ...
        block_size, sm_scale, block_M_fwd, block_M_bwd, mask_last_token, dtype, accum_dtype, num_threads_fwd, num_threads_bwd,
        enable_inverse_rope,
        cos, sin, is_training,
    ):
    ...
    def backward(ctx, do):
        ...
        # dw = torch.zeros((B, L, H, S), dtype=weights.dtype, device=weights.device)
        dw = torch.zeros((B, L, H, num_weights), dtype=weights.dtype, device=weights.device)
        ...
        return dq, dk, dv, dw, None, None, None, None, None, None, None, None, None, None, None, None, None, None


def HSA_block_M_group(
    q, k, v, weights, indices,
    block_size=64, sm_scale=None, block_M=None, block_M_fwd=None, block_M_bwd=None,
    ...
    # 兼容旧接口：如果只传了 block_M，则 fwd/bwd 都用 block_M
    if block_M_fwd is None:
        block_M_fwd = block_M
    if block_M_bwd is None:
        block_M_bwd = block_M
    # 兼容旧接口：如果只传了 num_threads，则 fwd/bwd 都用 num_threads
...
def HSA_block_M_inverse(q, k, v, weights, indices, block_size=64, sm_scale=None, block_M=0):
...
def main_block_M_correctness():
    ...
    # ---------- 配置参数 ----------
    B, SEQ_LEN, H, HQ, D, S, block_size = 1, 512, 1, 8, 128, 4, 64
    ...
    # HSA_pair 前向
    O_hsa = HSA_block_M_group(Q, K, V, W, block_indices, block_size=block_size, sm_scale=scale, block_M=block_M, mask_last_token=mask_last_token)
...
def main_block_M_latency():
    ...
    # ---------- 配置参数 ----------
    B, SEQ_LEN, H, HQ, D, S, block_size = 16, 8192, 4, 16, 64, 16, 64
...
def main_rope_correctness():
    ...
    weights = torch.randn((B, L, H, S), dtype=dtype, device=device)
    valid_mask = (block_indices != -1)
    weights_base = F.softmax(weights.masked_fill(~valid_mask, float('-inf')), dim=-1)
    weights_base = torch.nan_to_num(weights_base, nan=0.0).detach().requires_grad_(True)
...
def main_rope_latency_memory():
    ...
    def create_inputs(requires_grad=True):
        ...
        W = torch.nan_to_num(W, nan=0.0).detach().requires_grad_(requires_grad)
    ...
    # Warmup
    for _ in range(num_warmup):
        O = HSA_block_M_group(Q, K, V, W, block_indices, 
    ...
    O1 = HSA_block_M_group(Q1, K1, V1, W1, block_indices, 
                         block_size=block_size, sm_scale=scale, 
                         block_M=block_M, mask_last_token=mask_last_token,
                         enable_inverse_rope=False)
    ...
    # Warmup
    for _ in range(num_warmup):
        O = HSA_block_M_group(Q_rope, K_rope, V, W, block_indices, 
                            block_size=block_size, sm_scale=scale, 
                            block_M=block_M, mask_last_token=mask_last_token,
    ...
    O2 = HSA_block_M_group(Q_rope2, K_rope2, V2, W2, block_indices, 
                         block_size=block_size, sm_scale=scale, 
                         block_M=block_M, mask_last_token=mask_last_token,
                         enable_inverse_rope=True, cos=cos, sin=sin)
...
def test_correctness_fp32(B, SEQ_LEN, H, HQ, D, S, block_size):
    ...
    # 3. 前向计算
    O_hsa = HSA_block_M_group(Q, K, V, W, block_indices, block_size, scale, block_M, mask_last_token, "float", "float", 64)
    ...
    # torch.testing.assert_close(DW_hsa, W.grad, atol=0.01, rtol=0.01, msg="DW mismatch")
...
# python ops/hsa_fwd_bwd_group_forbench.py
