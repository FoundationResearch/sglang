...
Flow:
  ...
  3. Directly transform src_sd into converted_sd (rename / split keys).
  4. Save converted_sd.
  5. Create HSA model, load converted_sd with strict=False, report missing keys.
"""
...
def is_full_hsa(config: Dict[str, Any]) -> bool:
    ...
    When full HSA, there are no SWA q/k/v_proj in HSA layers;
    only hsa_q/k/v_proj exist.
    """
...
def normalize_key(key: str) -> str:
    """Normalize source key to match HSAForCausalLM state_dict naming.

    HSAForCausalLM structure:
      self.model = HSAModel(...)   -> keys: model.layers.X, model.embed_tokens, model.norm, ...
    ...
    # lm_head is a top-level param in HSAForCausalLM (not under self.model)
    if key.startswith("model.lm_head."):
        return key[len("model."):]  # model.lm_head.X -> lm_head.X
    if key.startswith("lm_head."):
        return key  # already correct

    # All other keys should have "model." prefix (they belong to HSAModel)
...
# ---------------------------------------------------------------------------
# 4. Rename mapping for full HSA (hsa_denom == 1)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 5. Build converted_sd directly from src_sd
# ---------------------------------------------------------------------------
...
def build_converted_sd(
    ...
    For HSA layers:
      - full HSA: rename q_proj -> hsa_q_proj, k_proj -> hsa_k_proj, v_proj -> hsa_v_proj
      - split HSA: split heads into SWA + HSA parts
    """
    ...
    for nk, tensor in normalized_src.items():
        # Check if this key belongs to an HSA layer's self_attn q/k/v/o_proj
        layer_idx = _extract_layer_idx(nk)
        if layer_idx is not None and layer_idx in lhsa_set:
            suffix = _extract_attn_suffix(nk)
            if suffix is not None:
                # This key is an attention projection in an HSA layer
                # Will be handled in the HSA conversion pass below
                continue
    ...
    for layer_idx in lhsa_indices:
        ...
        if src_q is None or src_k is None or src_v is None or src_o is None:
            raise KeyError(
                f"Cannot find q/k/v/o_proj weights for HSA layer {layer_idx}. "
                f"Available keys with this prefix: "
                f"{[k for k in normalized_src if k.startswith(prefix)]}"
            )

        if full_hsa_mode:
            # Full HSA (hsa_denom == 1): simple rename
            converted_sd[f"{prefix}hsa_q_proj.weight"] = src_q
            converted_sd[f"{prefix}hsa_k_proj.weight"] = src_k
            converted_sd[f"{prefix}hsa_v_proj.weight"] = src_v
            ...
            # Split HSA (hsa_denom > 1): split heads
            ...
            swa_q_indices = list(range(n_heads - hsa_heads))
            hsa_q_indices = list(range(n_heads - hsa_heads, n_heads))
            swa_kv_indices = sorted(set(i // q_per_kv for i in swa_q_indices))
            hsa_kv_indices = sorted(set(i // q_per_kv for i in hsa_q_indices))

            # Q: split
            converted_sd[f"{prefix}q_proj.weight"] = extract_head_rows(src_q, swa_q_indices, head_dim)
            converted_sd[f"{prefix}hsa_q_proj.weight"] = extract_head_rows(src_q, hsa_q_indices, head_dim)

            # K: SWA gets corresponding kv heads, HSA gets mean-pooled
            converted_sd[f"{prefix}k_proj.weight"] = extract_head_rows(src_k, swa_kv_indices, head_dim)
            converted_sd[f"{prefix}hsa_k_proj.weight"] = mean_pool_heads(src_k, hsa_kv_indices, h_hsa_kv, head_dim)

            # V: same as K
            converted_sd[f"{prefix}v_proj.weight"] = extract_head_rows(src_v, swa_kv_indices, head_dim)
            converted_sd[f"{prefix}hsa_v_proj.weight"] = mean_pool_heads(src_v, hsa_kv_indices, h_hsa_kv, head_dim)

            # O: permute columns (SWA heads first, HSA heads last)
            perm_order = swa_q_indices + hsa_q_indices
            col_indices = []
            for i in perm_order:
                col_indices.extend(range(i * head_dim, (i + 1) * head_dim))
            converted_sd[f"{prefix}o_proj.weight"] = src_o[:, col_indices]

            hsa_layer_logs.append({
                "layer_idx": layer_idx,
                "mode": "split_hsa",
                "swa_q_heads": swa_q_indices,
                "hsa_q_heads": hsa_q_indices,
                "swa_kv_heads": swa_kv_indices,
...
def main():
    ...
    if lhsa_indices:
        ...
        mode_str = "full_hsa_rename" if full_hsa_mode else f"split_hsa (hsa_denom={hsa_denom})"
    ...
    # ---- Step 4: Pad vocab & save converted state dict ----
    # Create HSA model first to get the actual vocab_size (may differ due to
    # landmark token insertion + alignment, e.g. 100278 -> 100288)
    print(f"[4/5] Creating HSA model, padding vocab weights, and saving...")
    model = build_foundation_model(config_path=args.target_config)

    # Resize vocab-related weights if vocab_size differs (e.g. 100278 -> 100288)
    # Extra rows are randomly initialized following _init_weights in modeling
