...
def ref_softmax_topk_max_pooling(q, k_lmks, lse_swa, topk, block_size, window_size, is_causal=False):
    """
    ...
    if is_causal:
        ...
        # Mask out chunks that are covered by the sliding window
        # Condition: chunk_idx >= floor((t_q - window_size + 1) / block_size)
...
def hsa_lse_kernel(
    batch, seq_len, s_len, h_kv, groups, head_dim, block_size, window_size,
    is_causal, 
    ...
    # LSE Out: [B, L, h_kv, G]
    lse_shape = [batch, seq_len, h_kv, groups]
    ...
    def kernel(
        ...
        with T.Kernel(tilelang.cdiv(seq_len, BLOCK_L), h_kv, batch, threads=threads) as (bx, by, bz):
            ...
            loop_limit = tilelang.cdiv(s_len, BLOCK_S)
            if is_causal:
                loop_limit = T.min(loop_limit, tilelang.cdiv(base_l + BLOCK_L, BLOCK_S))
...
def weighted_select_kernel(
    batch, seq_len, s_len, h_kv, groups, head_dim, topk, block_size, window_size,
    is_causal, 
    BLOCK_L=None, BLOCK_S=None, threads=None
):
    dtype = "bfloat16"
    ...
    def kernel(
        ...
        with T.Kernel(tilelang.cdiv(seq_len, BLOCK_L), h_kv, batch, threads=threads) as (bx, by, bz):
            ...
            if tx < BLOCK_L and (base_l + tx) < seq_len:
...
class SoftmaxTopKMaxPoolingFusedFn(torch.autograd.Function):
    ...
    def forward(ctx, q, lmks, lse_swa,
        ...
        indices_raw = select_kernel(q_in, k_in, lse_total)  # int32
...
class SoftmaxTopKMaxPooling_Fused(torch.nn.Module):
    ...
    def forward(self, q, lmks, lse_swa):
        # q:   [B, L, h_kv, G, D]
        # lmks:[B, S, h_kv, D]
        # lse_swa: [B, L, h_q]
        B, L, h_kv, G, D = q.shape
        ...
        if self._cached_shape != shape_key:
            ...
            self._cached_lse_kernel = hsa_lse_kernel(
                B, L, S, h_kv, G, D, block_size, window_size, is_causal
            )
...
def online_softmax_topk_head(q: torch.Tensor, lmks: torch.Tensor, lse_swa: torch.Tensor, 
    ...
    if cache_key not in _SOFTMAX_MODULE_CACHE:
        _SOFTMAX_MODULE_CACHE[cache_key] = SoftmaxTopKMaxPooling_Fused(topk, block_size, window_size, is_causal)

    return _SOFTMAX_MODULE_CACHE[cache_key](q, lmks, lse_swa)
