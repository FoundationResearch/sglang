...
def hierarchical_sparse_attention_block_M(batch, heads, q_len, kv_len, head_dim,
                                          ...
                                          selected_blocks=16, num_weights=None,  block_M = 64, mask_last_token=False, 
...
def hierarchical_sparse_attention_bwd_dqkv_block_M(
    ...
    block_M = 0, mask_last_token=False, window_size = -1, dtype="bfloat16", accum_dtype="float", num_threads = 256
):
    enable_last_token_mask = False
    if mask_last_token:
        enable_last_token_mask = True
...
def HSA_block_M_head_dense(Q, K, V, W, block_size=64, sm_scale=None, block_M=64, mask_last_token=False, 
                           window_size=-1,
...
def benchmark_dense_interface_weight_methods(
    ...
    assert L % block_size == 0, "L 必须能被 block_size 整除"
    device = "cuda"
    torch.manual_seed(0)
