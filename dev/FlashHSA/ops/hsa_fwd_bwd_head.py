...
#     import torch.nn.functional as F
#     from einops import rearrange
#     # from ops.hsa_fwd_bwd_triton import HSA # Commented out if not available
...
#     # [Testing Plan] 设置 num_weights 比 S 大，模拟包含 LSE/Sink 的情况
#     num_weights = S + 2 
#     print(f"Correctness Config: Batch={B}, SeqLen={SEQ_LEN}, H={H}, HQ={HQ}, D={D}, G={G}, S={S}, BlockSize={block_size}")
#     print(f"Testing with num_weights={num_weights} (S={S} + 2 extra)")
...
#     # [修改] 使用 num_weights 生成权重
#     logits = torch.randn((B, SEQ_LEN, HQ, num_weights), dtype=dtype, device=device)
#     valid_mask = (block_indices != -1) # Shape [B, L, H, S]
#     valid_mask_expanded = valid_mask.repeat_interleave(G, dim=2) # [B, L, HQ, S]
    
#     # 仅屏蔽前 S 列的无效位置
#     logits_s = logits[..., :S]
#     logits_s.masked_fill_(~valid_mask_expanded, -1e9)
#     logits[..., :S] = logits_s
...
#     # HSA Forward: Pass full W
#     O_hsa = HSA_block_M_head(Q, K, V, W, block_indices, block_size=block_size, sm_scale=scale, block_M=block_M, mask_last_token=mask_last_token)
    
#     # Ref Forward: Pass W sliced to S
#     O_ref = hsa_torch_ref(
#         Q.float().detach(), 
#         K.float().detach(), 
#         V.float().detach(), 
#         W.detach()[..., :S],  # Slice here!
#         block_indices,
#         chunk_size=block_size, 
#         sm_scale=scale, 
#         block_q=1,
...
#     # ====== Ref Backward ======
#     # Create isolated leaf for ref partial weights
#     W_ref_input = W.detach()[..., :S].clone().float().requires_grad_(True)
    
#     O_ref_bwd= hsa_torch_ref(
#         Q.float(), K.float(), V.float(), W_ref_input, block_indices,
#         chunk_size=block_size, sm_scale=scale, block_q=1, mask_last_token=mask_last_token
#     )
#     O_ref_bwd.backward(grad_output.float())
...
#     DK_ref = K.grad.clone()
#     DV_ref = V.grad.clone()
#     DW_ref_partial = W_ref_input.grad.clone() # Gradient for first S columns
...
#     DK_hsa = K.grad.clone()
#     DV_hsa = V.grad.clone()
#     DW_hsa = W.grad.clone() # Gradient for full W
...
#         max_diff = diff.max().item()
#         print(f"{name}最大误差: {max_diff:.6e}")
#         # ... (sample logic omitted) ...
#         return max_diff

#     compute_grad_diff(DQ_hsa, DQ_ref, "DQ")
#     compute_grad_diff(DK_hsa, DK_ref, "DK")
#     compute_grad_diff(DV_hsa, DV_ref, "DV")
    
#     # Check DW part 1 (First S columns)
#     print("\n--- Checking Weights Gradient ---")
#     dw_diff_part1 = compute_grad_diff(DW_hsa[..., :S], DW_ref_partial, "DW (cols 0:S)")
    
#     # Check DW part 2 (Extra columns) - MUST BE ZERO
#     extra_grad_norm = DW_hsa[..., S:].norm().item()
#     print(f"DW (cols S:end) Norm: {extra_grad_norm:.6e} (Should be 0.0)")
#     if extra_grad_norm > 1e-6:
#         print("FAILED: Gradient leakage to unused weights!")
#     else:
#         print("PASSED: Unused weights have zero gradient.")
...
def main_rope_correctness():
    ...
    valid_mask = (block_indices != -1)
    weights_base = F.softmax(weights.masked_fill(~valid_mask, -1e9), dim=-1)
...
def main_rope_latency_memory():
    ...
    def create_inputs(requires_grad=True):
        ...
        W = F.softmax(logits, dim=-1).requires_grad_(requires_grad)
...
def test_correctness_fp32(B, SEQ_LEN, H, HQ, D, S, block_size):
    ...
    logits = torch.randn((B, SEQ_LEN, HQ, S), dtype=dtype, device=device)
    logits.masked_fill_(block_indices.repeat_interleave(HQ//H, dim=2) == -1, -1e9)
    W = F.softmax(logits, dim=-1).detach().requires_grad_(True)
    grad_output = torch.randn((B, SEQ_LEN, HQ, D), dtype=dtype, device=device)
