...
#     - indices: (B, q_blocks, H, K) 或 (B, L, H, K) 且 L == q_blocks * block_q
#     - 返回: 
#         o_ref: (B, L, HQ, D) float32
#         lse_ref: (B, L, HQ, K) float32  <-- 新增返回
#     """
#     B, L, HQ, D = q.shape
#     H = k.shape[2]
#     G = HQ // H
...
#     # b: Batch
#     # q: Query Blocks
#     # x: Query Block Size (block_q)
#     # s: Key Block Size (chunk_size) -> 对应 dim=3
#     # k: Selected Blocks (K) -> 对应 dim=4
#     # h: Heads (HQ)
#     qk = torch.einsum('b q x h d, b q s k h d -> b q x s k h', q_chunked.float(), k_.float())
#     qk = qk * float(sm_scale)
...
#     # --- 新增 LSE 计算 ---
#     # 在 dim=3 (Key Block Size) 上做 LogSumExp
#     lse_ref = torch.logsumexp(qk, dim=3) # Shape: (B, Bq, X, K, HQ)
...
#     # --- 调整 LSE 形状 ---
#     # (B, Bq, X, K, HQ) -> (B, L, HQ, K)
#     lse_ref = rearrange(lse_ref, 'b q x k h -> b (q x) h k')
    
#     return o_ref.to(torch.float32), lse_ref.to(torch.float32)
...
def hierarchical_sparse_attention_block_M(batch, heads, q_len, kv_len, head_dim,
    ...
    def hsa_block_M(
        ...
        with T.Kernel(tilelang.cdiv(q_len, M), NV, batch * head_kv, threads=threads) as (bx, by, bz):
            ...
            for q_idx2, s_idx in T.Parallel(M, S):
                tq = base_t + q_idx2
                if tq < q_len:
                    W_local_shared[q_idx2, s_idx] = W[i_b, tq, i_h, s_idx]
                ...
                for _ in T.serial(S * M):
                    ...
                    for q_idx5 in T.serial(M):
                        if cur_val[q_idx5] == min_val:
                            ...
                            chunk_weights[k, q_idx5] = W_local_shared[q_idx5, pointers[q_idx5]]
                            # 保存原始 s_idx
            ...
            for i in T.serial(merged_len_local):
                ...
                for r in T.Parallel(M_G):
                    ...
                    if (ownership & (1 << q_idx)) != 0:
                        weight_vec[r] = chunk_weights[i, q_idx]
                    else:
                        weight_vec[r] = 0.0
...
#                     # 旧实现，适用于 tilelang 0.1.6.post2     
#                     # # Step 3: Copy small result to Shared Memory (为了支持 h_start + g 的动态索引)
#                     # T.copy(dw_row_sum_frag, dw_row_sum_shared)
#                     # # Step 4: Final Reduction over G and Write Output
#                     # for q_idx in T.serial(M):
#                     #     if (ownership & (1 << q_idx)) != 0:
#                     #         h_start = q_idx * G
#                     #         # 从 Shared 读取 G 个值到 Fragment 进行最后的规约
#                     #         for g in T.Parallel(G):
#                     #             dw_g_frag[g] = dw_row_sum_shared[h_start + g]
...
#                     # 新实现，只适用于 tilelang 0.1.7.post1
#                     T.copy(dw_row_sum_frag, dw_row_sum_shared)  # 这个拷贝虽然没用，但是可以加速，所以保留，可能因为触发了布局变换
#                     # Step 3: Reshape and reduce over G dimension
#                     dw_row_sum_frag_2d = T.reshape(dw_row_sum_frag, [M, G])
#                     # Create fragment for reduction results (per query)
#                     dw_row_sum_M = T.alloc_fragment([M], accum_dtype)
#                     # Reduce over G dimension (dim=1)
#                     T.reduce_sum(dw_row_sum_frag_2d, dw_row_sum_M, dim=1, clear=True)
#                     # Step 4: Write output for valid queries
#                     for q_idx in T.Parallel(M): 
#                         if (ownership & (1 << q_idx)) != 0:
#                             tq = base_t + q_idx
#                             s_idx_local = merged_s_indices[i, q_idx]
#                             if tq < q_len and s_idx_local != -1:
#                                 DW[i_b, tq, i_h, s_idx_local] = dw_row_sum_M[q_idx]
                ...
                if blk_idx >= 0:
                    ...
                    for step_idx in T.serial(num_inner_steps):
                        ...
                        for r, s in T.Parallel(M_G, step_S):
                            ...
                            qk_frag[r, s] = qk_frag[r, s] * chunk_weights[i, q_idx]
                    ...
                    for step_idx in T.serial(num_inner_steps):
                        ...
                        for r, s in T.Parallel(M_G, step_S):
                            q_idx = r // G
                            dS_frag[r, s] = sm_scale * (P_frag[r, s] * (chunk_weights[i, q_idx] * dP_frag[r, s] - delta_rows[r]))
