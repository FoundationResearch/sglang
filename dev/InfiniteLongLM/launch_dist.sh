...
#
# 日志:
#   rank-0: log_rank0.txt（同时打印到终端）
#   rank-1~N: log_rank1.txt, log_rank2.txt ...（在 WORK_DIR 下）
#
...
for i in $(seq 1 $((NNODES - 1))); do
    ...
    ssh_exec "$ip" \
        "export PATH='${RANK0_PATH}'; \
         ...
         cd ${WORK_DIR} && \
         nohup bash ${TRAINING_SCRIPT} > ${WORK_DIR}/log_rank${i}.txt 2>&1 & \
...
bash "${TRAINING_SCRIPT}" 2>&1 | tee "${WORK_DIR}/log_rank0.txt"
