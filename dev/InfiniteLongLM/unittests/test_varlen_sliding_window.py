...
def _build_model(device: str, dtype: torch.dtype, sliding_window: int) -> HSAForCausalLM:
    config = HSAConfig(
        vocab_size=128,
        hidden_size=64,
    ...
    model = HSAForCausalLM(config).to(device=device, dtype=dtype)
    model.auto_insert_lmk = True
    model.eval()
    return model

def _build_hybrid_model(device: str, dtype: torch.dtype, sliding_window: int) -> HSAForCausalLM:
    config = HSAConfig(
        vocab_size=128,
        hidden_size=256,
...
def _run(model: HSAForCausalLM, input_ids: torch.Tensor, position_ids: torch.Tensor = None, cu_seq_lens_q=None):
