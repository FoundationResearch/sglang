...
# Dash Attention training kernels.
# - Stage 1: `compressed_attention` (autograd.Function with fwd+bwd over
#   compressed K). NOTE: the package's `__init__.py` re-exports this same
#   differentiable wrapper under the misleading alias `compressed_attention_fwd`;
#   we import the real name from `stage1` directly to make the autograd path
#   obvious to readers.
# - `transform_score`: add attention sinks + sliding window + topk preparation.
# - `topk_sparse_attention`: Stage 2 forward+backward sparse attention kernel.
# - `triton_entmax` (from adasplash): per-head entmax used inside Stage 1.
# These are open-sourced under `dash-attention/infllmv2_entmax/`; the host repo
# must add that path to PYTHONPATH so `infllmv2_entmax` is importable.
# Capture the underlying error so we can show *why* the kernels could not be
# loaded. Some failures are not pure ImportError (e.g. triton/cuda mismatches
# raise RuntimeError at module import time), so widen to Exception.
...
def _dash_compressed_attention(
    ...
    # `compressed_attention` is the autograd.Function wrapper from
    # infllmv2_entmax/stage1.py: it backprops dq/d_compressed_k through the
    # custom triton bwd kernel, so Stage 0 (CompressK) and `indexer_q` actually
    # receive gradients via this path.
    ...
    # Per-query-head entmax then mean-pool across heads inside the same KV group.
    # NOTE: adasplash's `_bwd_entmax` uses `tl.sqrt(y)` on the saved entmax
    # output, which only accepts fp32/fp64. Cast the score to fp32 here so the
    # backward kernel can compile under bf16 mixed precision training.
...
class SWANDashModel(Qwen3PreTrainedModel):
    """
...
class SWANDashForCausalLM(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = SWANDashModel(config)
        self.vocab_size = config.vocab_size
...
__all__ = ["SWANDashForCausalLM", "SWANDashModel", "Qwen3PreTrainedModel"]
