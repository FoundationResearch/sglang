...
class HierarchicalSparseAttention(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.cross_pre_norm = Qwen3RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.cross_q_proj = nn.Linear(self.d_model, self.d_model)
        self.cross_o_proj = nn.Linear(self.d_model, self.d_model)

        self.topk = config.hsa_topk
        self.window_size = config.sliding_window
        self.chunk_size = config.chunk_size
        self.q_norm = Qwen3RMSNorm(self.head_dim)
        self.cross_q_norm = Qwen3RMSNorm(self.head_dim)
    ...
    def forward(self,
        ...
        **kwargs,
        ...
        swa_o = rearrange(swa_o, 'B L h d->B L (h d)')
        residual = residual + self.o_proj(swa_o)
        hsa_input = self.cross_pre_norm(residual)
        cross_q = self.cross_q_proj(hsa_input)
        cross_q = rearrange(cross_q, 'B L (h d)->B L h d', d=self.head_dim)
        hsa_q_norm = self.cross_q_norm(cross_q)
        
        indices, scores = self.topk_func(hsa_q_norm, chunk_lmks, self.topk, self.chunk_size, window_size=self.window_size, is_causal=True)  # (B, L, h_q, k), scores should be differential
        # indices: [B, L, h_kv, K], scores: [B, L, h_kv, K]

        
        chunk_weights = F.softmax(scores, dim=-1)
        ...
        hsa_o = rearrange(hsa_o, 'B L h d->B L (h d)')
        

        return residual + self.cross_o_proj(hsa_o) - org_residual, None
