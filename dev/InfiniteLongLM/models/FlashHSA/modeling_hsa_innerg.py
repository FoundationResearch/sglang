...
class HierarchicalSparseAttention(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.lmk_q_proj = nn.Linear(self.d_model, max(1, self.d_kv // 2) * self.head_dim, bias=False)
    ...
    def forward(self,
        ...
        **kwargs,
        ...
        k = rearrange(k, 'B L (h d)->B L h d', d=self.head_dim)  # (B, L, h, d)
        k_norm = self.k_norm(k)
        v = rearrange(v, 'B L (h d)->B L h d', d=self.head_dim)  # (B, L, h, d)

        assert q_norm.shape[2] % 2 == 0, 'q_norm must have even number of heads'
        q_norm_swa, q_norm_hsa = torch.chunk(q_norm, 2, dim=2)
        if self.h_kv == 1:
            swa_k_norm, hsa_k_norm = k_norm, k_norm
            swa_v, hsa_v = v, v
        else:
            assert self.h_kv % 2 == 0, 'h_kv must have even number of heads or ==1'
            swa_k_norm, hsa_k_norm = torch.chunk(k_norm, 2, dim=2)
            swa_v, hsa_v = torch.chunk(v, 2, dim=2)
        ...
        q_norm_swa = q_norm_swa.transpose(1, 2)
        swa_k_norm = swa_k_norm.transpose(1, 2)
        swa_v = swa_v.transpose(1, 2)  # (B, h, L, d)
        cos, sin = position_embeddings
        q_norm_swa, swa_k_norm = apply_rotary_pos_emb(q_norm_swa, swa_k_norm, cos, sin)
        ...
        q_norm_hsa = q_norm_hsa.contiguous()
        hsa_k_norm = hsa_k_norm.contiguous()
        hsa_v = hsa_v.contiguous()
        swa_o, lse_sum = compiled_flex_attention(
            q_norm_hsa.transpose(1, 2), 
        ...
        indices, scores = self.topk_func(lmk_q_norm, lmk_k, self.topk, block_size=self.chunk_size, window_size=self.window_size, is_causal=True)
        ...
        if not self.enable_softmax1:
            ...
            # softmax off by one
            cat_scores = torch.cat([scores, lse_sum.unsqueeze(-1), torch.zeros(B, L, self.h_kv // 2, 1, device=hidden_states.device)], dim=-1)
            swa_weight_idx = -2
        chunk_weights = F.softmax(cat_scores, dim=-1).to(hidden_states.dtype) # (B, L, h_kv, K)

        hsa_o = self.hsa_func(q_norm_hsa, hsa_k_norm, hsa_v, weights=chunk_weights, indices=indices, block_size=self.chunk_size, mask_last_token=True)
