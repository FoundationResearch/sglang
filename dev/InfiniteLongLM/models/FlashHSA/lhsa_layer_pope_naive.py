        ...
        **kwargs,
        ...
        if full_seq_len >= self.chunk_size:
            ...
            if self.nope_chunkwise_attn or has_intra_pos:
                # (B, L, h, d) -> (B, h, L, d)
                hsa_q_norm = hsa_q_norm_nope.transpose(1, 2).contiguous()
                hsa_k_norm = hsa_k_norm_nope.transpose(1, 2).contiguous()
            else:
                hsa_q_norm = hsa_q_norm_pope
                hsa_k_norm = hsa_k_norm_pope
            ...
            if has_intra_pos:
                ...
                S, L_k = k_pos.shape[0], hsa_k_norm.shape[2]
                pos_idx = torch.arange(L_k, device=hsa_k_norm.device) % S
                # (L_k, h_kv, d) -> (1, h_kv, L_k, d) for broadcast over batch
                k_bias = k_pos[pos_idx].transpose(0, 1).unsqueeze(0)
