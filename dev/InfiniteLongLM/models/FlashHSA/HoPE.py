...
1. **Partial-RoPE via freq mask.**  Keep only the first
...
class HoPERotaryEmbedding(nn.Module):
    ...
    def __init__(self, config, device=None):
        ...
        # rope-scaling variants).  HoPE itself does no scaling.
        ...
        logger.warning_once(
            ...
            f"theta={self._compute_theta():.4e}, "
            f"shortest period = {2 * math.pi / inv_freq[0].item():.2f} tokens, "
            f"longest period = {2 * math.pi / inv_freq[n_keep - 1].item():.2f} "
