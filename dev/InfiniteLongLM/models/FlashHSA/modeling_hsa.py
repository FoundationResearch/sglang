...
class HierarchicalSparseAttention(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        self.lmk_norm = Qwen3RMSNorm(self.head_dim)
        
        hsa_mode = config.hsa_mode
        if hsa_mode == 'dense':
            from ops.hsa_fwd_bwd_head_dense import HSA_dense_interface as HSA
            self.hsa_func = HSA
        else:
    ...
    def forward(self,
        ...
        **kwargs,
        ...
        # 构建混合的key tensor：
        # - 非lmks位置: apply k_norm
        # - lmks位置: apply lmk_norm
        # 这样存入cache后，取出时直接按位置切片即可
        swa_k_norm = self.k_norm(swa_k)  # (B, L, h, d)
        
        # 对lmks位置单独apply lmk_norm，并替换到swa_k_norm中
        chunk_lmks_normed = self.lmk_norm(swa_k[:, self.chunk_size - 1::self.chunk_size, :, :])  # (B, num_chunks, h, d)
        
        # 创建混合key tensor
        swa_k_mixed = swa_k_norm.clone()
        swa_k_mixed[:, self.chunk_size - 1::self.chunk_size, :, :] = chunk_lmks_normed
        ...
        #     # 把当前的swa_k_mixed，swa_v 和之前的kv cache 拼接
        ...
        #     # 保存混合归一化后的kv
        #     key_states = swa_k_mixed.transpose(1, 2)
        #     value_states = swa_v.transpose(1, 2)
        #     key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
            
        #     # 取出完整的kv
        #     swa_k_mixed = key_states.transpose(1, 2).contiguous()
        #     swa_v = value_states.transpose(1, 2).contiguous()
            
        #     swa_k_norm = swa_k_mixed   
        ...
        if use_cache and past_key_value is not None: 
            ...
            cache_seq_len = past_key_value.get_seq_length(self.layer_idx)
            need_offload = cache_seq_len > -1  # 开启offloading的kv cache长度阈值

            # debug
            need_offload = False

            if need_offload and self.layer_idx < len(past_key_value.layers) and past_key_value.layers[self.layer_idx].keys is not None:
                past_key_value.layers[self.layer_idx].keys = past_key_value.layers[self.layer_idx].keys.to(current_device)
                past_key_value.layers[self.layer_idx].values = past_key_value.layers[self.layer_idx].values.to(current_device)
            
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
            # # ============ DEBUG: 检查 KV Cache 是否被 TP 切分 ============
            # # 只在第0层打印，避免输出过多
            # if self.layer_idx == 3:
            #     import torch.distributed as dist
            #     rank = dist.get_rank() if dist.is_initialized() else 0
            #     print(f"[DEBUG Layer {self.layer_idx}] Rank {rank}: "
            ...
            if need_offload:
                past_key_value.layers[self.layer_idx].keys = past_key_value.layers[self.layer_idx].keys.cpu()
                past_key_value.layers[self.layer_idx].values = past_key_value.layers[self.layer_idx].values.cpu()
        ...
        if shared_kv is not None:
            ...
            # hsa: (B, L, h_kv, d)
            # 获取完整序列的长度
            full_seq_len = swa_k_mixed.shape[1] if use_cache and past_key_value is not None else L
            
            # chunk_lmks 直接从 swa_k_mixed 中按位置切片
            # swa_k_mixed 中 lmks 位置已经是 lmk_norm 处理过的，可以直接使用
            sliced_k = swa_k_mixed[:, self.chunk_size - 1::self.chunk_size, :, :]  # (B, num_chunks, h_kv, d)

            chunk_lmks = torch.empty_like(sliced_k, memory_format=torch.contiguous_format)
            chunk_lmks.copy_(sliced_k)
        # print(f'layer_idx: {self.layer_idx} q_norm: {q_norm[0, -1, 0, :5]}, lse_sum: {lse_sum[:, -1, :]}')
        
        if self.hsa_mode == 'sparse':
            ...
            mode = 'inference' if use_cache else 'train'
            q_offset = int(cache_position[0].item()) if cache_position is not None else 0

            indices, scores = self.topk_func(q_norm, chunk_lmks, lse_sum, 
                            self.topk, self.chunk_size, window_size=self.window_size, is_causal=True, mode=mode, q_offset=q_offset)
            ...
            chunk_weights = F.softmax(cat_scores, dim=-1).to(torch.bfloat16) # (B, L, h_q, K)

            # hsa_o = self.hsa_func(q_norm, hsa_k_norm, hsa_v, chunk_weights, indices, mask_last_token=True)  # 这里chunk weights最后一维是K+1, 需要只读到 K - 1
            hsa_o = self.hsa_func(q_norm, hsa_k_norm, hsa_v, chunk_weights, indices, block_size=self.chunk_size, mask_last_token=True, mode=mode)
            # print(f'head 0 indices: {indices[:, -1, 0, :]}, weights: {chunk_weights[:, -1, 0, :]}')
            swa_o_weight = chunk_weights[:, :, :, swa_weight_idx]  # (B, L, h_q)
            # o = hsa_o + swa_o * swa_o_weight.unsqueeze(-1)
            o = torch.addcmul(hsa_o, swa_o, swa_o_weight.unsqueeze(-1))
            o = rearrange(o, 'B L h d->B L (h d)')
        elif self.hsa_mode == 'dense':
            # G = self.h_q // self.h_kv
            # chunk_lmks = torch.repeat_interleave(chunk_lmks, dim=2, repeats=G)  # (B, M, h_q, d)
            # scores = einsum('B L h d, B M h d-> B L h M', q_norm, chunk_lmks)  # (B, L, h_q, L // cz)
            # # causal masking
            # num_chunks = L // self.chunk_size
            # assert num_chunks == scores.shape[-1]
            # mask = create_chunk_causal_mask(L, num_chunks, self.chunk_size, self.window_size, scores.device)

            # # 扩展维度以匹配 scores: (1, L, 1, num_chunks)
            # mask = mask.unsqueeze(0).unsqueeze(2)
            # print(f'q_norm shape: {q_norm.shape} {q_norm.dtype}, ' +
            #       f'hsa_k_norm shape: {hsa_k_norm.shape}, {hsa_k_norm.dtype},' +
            #       f'hsa_v shape: {hsa_v.shape}, {hsa_v.dtype}, ' +
            #       f'lmk shape: {chunk_lmks.shape} {chunk_lmks.dtype}, lse_sum shape: {lse_sum.shape}, {lse_sum.dtype}')
            hsa_o, chunk_weights, lse_idx = self.hsa_func(q_norm, 
                                  hsa_k_norm, 
                                  hsa_v, 
                                  lmk=chunk_lmks, 
                                  lse_swa=lse_sum, 
                                  ...
                                  enable_softmax1=self.enable_softmax1)
            # print(f'weights: {chunk_weights[:, -1, 0, :]}')
            chunk_weights = chunk_weights.to(hidden_states.dtype)
            swa_o_weight = chunk_weights[:, :, :, lse_idx]
            o = torch.addcmul(hsa_o, swa_o, swa_o_weight.unsqueeze(-1))
            o = rearrange(o, 'B L h d->B L (h d)')
        else:
            raise Exception(f'Unknown HSA mode: {self.hsa_mode}')
        # print(f'layer_idx: {self.layer_idx} o: {o[0, -1, :5]}')
