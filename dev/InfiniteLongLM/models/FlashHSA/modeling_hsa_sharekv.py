...
class FlashHSA(nn.Module):
    ...
    def forward(self,
        ...
        window_size = torch.tensor(self.window_size, device=hidden_states.device)
        chunk_size = torch.tensor(self.chunk_size, device=hidden_states.device)

        def block_causal_mask(b, h, q_idx, kv_idx):
            # every token can see 1) at least window_size tokens before it 2) chunk_size * S tokens
            return (q_idx >= kv_idx) & (q_idx <= (kv_idx + window_size + chunk_size - 1) // chunk_size * chunk_size)
        ...
        indices, scores = self.topk_func(q_norm, chunk_lmks, lse_sum, self.topk, self.chunk_size, is_causal=True)  # (B, L, h_q, k), scores should be differential
        # indices: [B, L, h_kv, K], scores: [B, L, h_q, K]
        # print(f'indices shape: {indices.shape}, scores shape: {scores.shape}')
        scores = rearrange(scores, 'B L h G K->B L (h G) K')

        # TODO: assert indices <= q_chunk - x

        # lse_sum = lse_sum.transpose(1, 2)
        if not self.enable_softmax1:
            cat_scores = torch.cat(scores, lse_sum.unsqueeze(-1), dim=-1)  # (B, L, h_q, K + 1)
            swa_weight_idx = -1
        else:
            # softmax off by one
            # print(f'scores shape: {scores.shape}, lse_sum shape: {lse_sum.shape}')
            # print(f'scores dtype: {scores.dtype}, lse_sum dtype: {lse_sum.dtype}')
            cat_scores = torch.cat([scores, lse_sum.unsqueeze(-1), torch.ones(B, L, self.h_q, 1, device=hidden_states.device)], dim=-1)
            swa_weight_idx = -2

        chunk_weights = F.softmax(cat_scores, dim=-1).to(hidden_states.dtype) # (B, L, h_q, K)
        indices = indices.to(torch.int32)

        #  TODO: remove turncatin
        hsa_o = self.hsa_func(q_norm, hsa_k_norm, hsa_v, chunk_weights[:, :, :, :self.topk].contiguous(), indices, mask_last_token=True)  # 这里chunk weights最后一维是K+1, 需要只读到 K - 1
        swa_o_weight = chunk_weights[:, :, :, swa_weight_idx]
...
class SlidingWindowAttention(nn.Module):
    ...
    def forward(
        ...
        cos, sin = position_embeddings
        assert cos.shape[1] == query_states.shape[-2], f'cos.shape: {cos.shape}, q.shape: {query_states.shape}'
...
class DecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: LlamaConfig, attn_cls, layer_idx: int = None):
        super().__init__()
        self.layer_idx = layer_idx
...
class FlashHSAModel(FlashHSAPreTrainedModel):
    def __init__(self, config):
        ...
        for layer_idx in range(config.num_hidden_layers):
            if (1 + layer_idx) % config.full_attn_interleave == 0:
                attn_cls = FlashHSA
            else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.training:
            assert input_ids.shape[-1] % self.sliding_window == 0
            for chunk_ids in input_ids[:, ::self.self.sliding_window]:
