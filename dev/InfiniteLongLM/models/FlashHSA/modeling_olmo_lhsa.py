...
from .lhsa_layer import LandmarkHSA as LandmarkHSA_base
...
@dataclass
class GenerateState:
    """生成模式的状态管理，封装 decode 过程中的所有状态变量。"""
    active: bool = False                          # 是否处于 generate 模式
    decode_token_count: int = 0                    # 当前 chunk 内已 decode 的 token 数
    next_pos: int = 0                              # 下一个 token 的 position id
    cache_seq_len: int = 0                         # 含 LMK 的 KV cache 长度，用于正确计算 cache_position
    lmk_positions_in_input: Optional[list] = None  # 当前输入中 LMK token 的位置列表

    def reset(self):
        """重置所有状态，用于 generate 结束后清理。"""
        self.active = False
        self.decode_token_count = 0
        self.next_pos = 0
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if chunk_prefill_threshold > 0 and not self.training:
            ...
            if _seq_len > chunk_prefill_threshold:
                # debug
                # print(f"Input sequence length {_seq_len} exceeds chunk_prefill_threshold {chunk_prefill_threshold}, using chunked forward.")
...
class HSAForCausalLM(Olmo3PreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(
        ...
        if gs.decode_token_count < self.chunk_size - 2:
            ...
            cache_position = torch.tensor([gs.cache_seq_len], device=input_ids.device)
            ...
            cache_position = torch.tensor([gs.cache_seq_len, gs.cache_seq_len + 1], device=input_ids.device)
