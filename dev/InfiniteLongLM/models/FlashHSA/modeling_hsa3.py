...
class HierarchicalSparseAttention(nn.Module):
    def __init__(self, config, layer_idx):
        ...
        # self.lmk_q_proj = nn.Linear(self.d_model, self.d_model)
        self.lmk_q_proj = nn.Sequential(nn.Linear(self.d_model, config.lmk_dim, bias=False), 
                            nn.Linear(config.lmk_dim, self.d_model, bias=False))
    ...
    def forward(self,
        ...
        **kwargs,
        ...
        lmk_q = self.lmk_q_proj(hidden_states) + q
        ...
        if use_cache and past_key_value is not None: 
            ...
            key_states = swa_k_norm.transpose(1, 2)
            ...
            # # ============ END DEBUG ============
            swa_k_norm = key_states.transpose(1, 2).contiguous()
        ...
        if shared_kv is not None:
            ...
            # hsa: (B, L, h_kv, d)
            full_seq_len = swa_k_norm.shape[1] if use_cache and past_key_value is not None else L
            
            sliced_k = swa_k_norm[:, self.chunk_size - 1::self.chunk_size, :, :]  # (B, num_chunks, h_kv, d)
        ...
        indices, scores = self.topk_func(lmk_q_norm, chunk_lmks, lse_sum, 
