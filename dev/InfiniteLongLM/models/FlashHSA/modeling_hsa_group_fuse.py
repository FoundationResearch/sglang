...
class HierarchicalSparseAttention(nn.Module):
    ...
    def forward(self,
        ...
        **kwargs,
        ...
        lse_sum = rearrange(lse_sum, 'B L (h G)->B L h G', G=self.h_q // self.h_kv)
        lse_sum = lse_sum.sum(dim=-1).contiguous()  # sum along group
        ...
        indices, scores = self.topk_func(q_norm, chunk_lmks, self.topk, self.chunk_size, window_size=self.window_size, is_causal=True)  # (B, L, h_q, k), scores should be differential
        # indices: [B, L, h_kv, K], scores: [B, L, h_kv, K]
        ...
        if not self.enable_softmax1:
            ...
            # softmax off by one
            cat_scores = torch.cat([scores, lse_sum.unsqueeze(-1), torch.zeros(B, L, self.h_kv, 1, device=hidden_states.device)], dim=-1)
            swa_weight_idx = -2
        ...
        swa_o_weight_expanded = swa_o_weight.repeat_interleave(self.h_q // self.h_kv, dim=2) 
        o = torch.addcmul(hsa_o, swa_o, swa_o_weight_expanded.unsqueeze(-1))
