...
def hsa_torch_ref(q, k, v, weights, indices, *, chunk_size: int, sm_scale: float, block_q: int, mask_last_token: bool = True, slope=None):
    ...
    # Intra-chunk ALiBi.  Distance over k_intra=0..chunk_size-1 is
    #   [S-2, S-3, ..., 1, 0, 0]
    # i.e. the standard "anchor at last token" pattern with TWO zero tails
    # (the very last K token is reserved as a special / landmark token, so
    # positions S-1 and S-2 are equally close to the q anchor).
    # Closed form: ``dist[c] = max(0, (chunk_size - 2) - c)``.
    #
    # IMPORTANT: the kernel applies this bias on RAW ``q.k`` (before
    # multiplying by ``sm_scale`` inside exp2) so that the softmax sees an
    # effective bias of ``slope * dist * sm_scale``.  We therefore add the
    # bias BEFORE the ``qk *= sm_scale`` step to match that behavior.
        ...
        with T.Kernel(tilelang.cdiv(q_len, M), batch * head_kv, threads=num_threads) as (bx, bz):
            ...
            for i in T.Pipelined(merged_len_local, num_stages=num_stages):
                ...
                if (blk_idx >= 0):
                    ...
                    # Intra-chunk ALiBi: subtract slope * dist[c], where the
                    # distance vector over c=0..S-1 is
                    #   [S-2, S-3, ..., 1, 0, 0]
                    # i.e. the standard "anchor at the last token" pattern but
                    # with TWO zero tails (the very last K token is reserved
                    # as a special / landmark token, so we treat positions
                    # ``S-1`` and ``S-2`` as equally close to the q anchor).
                    # Closed form:  dist[c] = max(0, (S-2) - c).
                    # Inlined (see fwd comment about variable rebinding).
                    if enable_alibi:
        ...
        with T.Kernel(tilelang.cdiv(q_len, M), B * heads_kv, threads=num_threads) as (bx, bz):
            ...
            for i in T.Pipelined(merged_len_local, num_stages=num_stages):
                ...
                if blk_idx >= 0:
                    ...
                    # Intra-chunk ALiBi: must apply the SAME bias as forward so
                    # that the recomputed softmax probabilities match.
                    # ScoresLSE was saved with the ALiBi bias already folded in,
                    # so subtracting the same ``slope * dist[s]`` here lets
                    # ``exp2(acc_s_tmp*scale_log2 - saved_lse)`` reproduce p.
                    # dist over s=0..S-1 is [S-2, S-3, ..., 1, 0, 0]
                    #   = max(0, (S-2) - s)  -- two-zero tail for special token.
                    # Inlined (see fwd comment about variable rebinding).
                    if enable_alibi:
...
class _hsa_block_M_attention_inverse(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, weights, indices,
        ...
        # ALiBi: per-q-head slope tensor.  If None, ALiBi is disabled and we
        # still pass a zero-filled placeholder into the kernel so it does not
        # need a separate compiled version without the arg; the branch inside
        # the kernel is gated by the compile-time flag ``enable_alibi``.
    ...
    def backward(ctx, do):
        if not getattr(ctx, "is_training", True):
            raise RuntimeError(
                "[_hsa_block_M_attention_inverse] forward 以 is_training=False 运行，"
                "未保存反向所需的 merged indices / ScoresLSE，无法执行 backward。"
            )
        ...
        # forward takes 15 args: q, k, v, weights, indices, block_size, sm_scale,
        # block_M, mask_last_token, dtype, accum_dtype, num_threads_fwd,
        # num_threads_bwd, is_training, slope -> backward must return 15 grads.
...
def HSA_block_M_head(
    ...
    """HSA block-M attention with optional intra-chunk ALiBi.

    ``slope``: optional tensor of shape ``(HQ,)`` giving a per-q-head ALiBi
    slope.  The intra-chunk bias added to the RAW (unscaled) ``q.k`` logits is
    ``-slope[h] * dist[k_intra]`` where ``k_intra in 0..S-1`` is the k token's
    chunk-local position and ``dist`` is the vector
        ``[S-2, S-3, ..., 1, 0, 0]``  (i.e. ``max(0, (S-2) - k_intra)``).
    Equivalently, the q anchor sits at position ``S-2``; the trailing two
    K positions are treated as equally close to q because the very last K
    token is reserved as a special / landmark token in the HSA design.
    After the standard ``sm_scale`` multiply inside exp2, the effective
    softmax bias is therefore ``-slope[h] * dist[k_intra] * sm_scale``,
    matching the conventional FlashAttention ALiBi semantic.  Pass ``None``
    to disable ALiBi (zero overhead kernel path).
    """
    B, L, HQ, D = q.shape
    _, L_kv, H, Dk = k.shape
    ...
    if tuned is not None:
        ...
        # if applied:
        #     print(f"[HSA_block_M_head tuned] G={G} -> {', '.join(applied)}")
...
def main_bwd_breakdown_latency(
    ...
    print()
    print("=" * 78)
    print("[HSA_block_M_head autograd]  走 autograd.Function 完整路径（单 kernel 反向）")
    print("=" * 78)
    ...
    if inv_results:
        ...
        print()
        print("=" * 78)
        print("🏆 最快配置（按不同指标，HSA_block_M_head autograd 完整路径）")
        print("=" * 78)
        def _fmt(r, tag):
...
if __name__ == "__main__":
    ...
    main_bwd_breakdown_latency(
        B=4, HQ=32, H=16, SEQ_LEN=8192,
        S=32, D=128, block_size=64,
        real_indices_path="/apdcephfs_fsgm/share_303843174/user/qqzxywei/wxy/real_indices_backup_headwise/indices_8192.pt",
        layer_idx=3,                    # ← 这里从 None 改成 3
        M_list=(16,),
        nt_fwd_list=(128,),
        nt_bwd_list=(128,),
        paired_only=False,
...
# pkill -f "burner.*--gpu 7"; export CUDA_VISIBLE_DEVICES=7; python ops/hsa_fwd_bwd_head_forbench.py