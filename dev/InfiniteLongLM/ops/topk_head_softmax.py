...
def ref_softmax_topk_max_pooling(q, k_lmks, lse_swa, topk, block_size, window_size, is_causal=False, q_offset=0):
    """
    ...
    if is_causal:
        ...
        i_idx_global = i_idx + q_offset  # ✅ 全局位置
    ...
    # 处理 topk > S 或 causal mask 导致有效元素不足的情况
    # 将 -inf 替换为一个很小的值，这样 topk 仍然可以工作
    # 但我们需要在最后标记这些无效的 indices
    actual_topk = min(topk, S)
    
    _, topk_indices = torch.topk(scores_max_pooling, k=actual_topk, dim=-1, sorted=False)
    
    # 如果 actual_topk < topk，需要 padding
    if actual_topk < topk:
    ...
    sort_temp[sort_temp < 0] = S + 1000  # 大于任何有效索引
    indices_sorted, order = torch.sort(sort_temp, dim=-1)
    indices_sorted[indices_sorted >= S] = -1  # 恢复 -1
    
    order_expanded = order.unsqueeze(3).expand(-1, -1, -1, G, -1)
    
    # 对于 indices 为 -1 的位置，使用 0 来 gather（避免越界），之后会 mask 掉
...
def hsa_lse_kernel(
    ...
    is_causal, use_cache=False, seq_len=None, s_len=None,
    ...
    def kernel(
        ...
        with T.Kernel(tilelang.cdiv(seq_len_var, BLOCK_L), h_kv, batch, threads=threads) as (bx, by, bz):
            # if use_cache:
            #     q_offset = Q_Offset[0]
            q_offset = T.if_then_else(use_cache, Q_Offset[0], 0)
            i_b, i_h = bz, by
            base_l = bx * BLOCK_L
            ...
            # [FIX 1] Allocate shared memory for scores
            ...
            for s_block in T.serial(loop_limit):
                ...
                # [FIX 2] Copy Fragment -> Shared -> Mask -> Copy Back
                ...
                if is_causal:
                    for i, j in T.Parallel(GEMM_M, GEMM_N):
                        ...
                        ts = base_s + j
                        if tq_local < seq_len_var and ts < s_len_var:
                ...
                for i in T.Parallel(GEMM_M):
                    # [FIX 3] Keep the NaN protection in Scale
...
def recompute_topk_max_pooling_scores_kernel(
    batch,  h_kv, groups, head_dim, topk, seq_len=None, s_len=None, use_cache=False, 
...
class SoftmaxTopKMaxPoolingFusedFn(torch.autograd.Function):
    ...
    def forward(ctx, q, lmks, lse_swa,
        ...
        # 计算 HSA LSE
        # lse_hsa: [B, L, h_kv, G]
        lse_hsa = lse_kernel(q_in, k_in, q_offset_tensor)
        ...
        indices_raw = select_kernel(q_in, k_in, lse_total, q_offset_tensor)  # int32
    ...
    def backward(ctx, grad_indices_unused, grad_scores_selected):
        ...
        return grad_q, grad_lmks, None, None, None, None, None, None, None


class SoftmaxTopKMaxPooling_Fused(torch.nn.Module):
    def __init__(self, topk, block_size, window_size, is_causal, use_cache=False):
        super().__init__()
        self.topk = topk
    ...
    def forward(self, q, lmks, lse_swa, q_offset):
        # q:   [B, L, h_kv, G, D]
        # lmks:[B, S, h_kv, D]
        # lse_swa: [B, L, h_q]
        B, L, h_kv, G, D = q.shape
        ...
        if use_cache:
            ...
            shape_key = (B, h_kv, G, D, topk, block_size, window_size, is_causal, use_cache)
        else:
            # 训练模式：L 和 S 是编译时常量
            shape_key = (B, L, S, h_kv, G, D, topk, block_size, window_size, is_causal, use_cache)
        
        if self._cached_shape != shape_key:
            # use_cache 时传 None，否则传具体值
            seq_len_param = None if use_cache else L
            s_len_param = None if use_cache else S
        ...
        indices, scores = SoftmaxTopKMaxPoolingFusedFn.apply(
            q, lmks, lse_swa, lse_kernel, select_kernel, sort_kernel, recompute_kernel, q_offset_tensor
        )
...
def online_softmax_topk_head(q: torch.Tensor, lmks: torch.Tensor, lse_swa: torch.Tensor, 
                             topk: int, block_size: int, window_size: int, is_causal: bool = False,
                             mode: str = 'train', q_offset: int = 0):
    """
    ...
    Args:
        ...
        window_size: int
        is_causal: bool
        mode: str - 'train' or 'inference'. 'inference' enables dynamic shape support.
        q_offset: int - Q 在全局序列中的起始位置（用于 chunk prefill 的 causal mask）
    ...
    use_cache = (mode == 'inference')
    cache_key = (topk, block_size, window_size, is_causal, use_cache)
    
    if cache_key not in _SOFTMAX_MODULE_CACHE:
        _SOFTMAX_MODULE_CACHE[cache_key] = SoftmaxTopKMaxPooling_Fused(topk, block_size, window_size, is_causal, use_cache)

    return _SOFTMAX_MODULE_CACHE[cache_key](q, lmks, lse_swa, q_offset=q_offset)
...
def test_fused_topk_softmax_max_pooling_correctness():
    ...
    is_causal = True
    block_size = 32
    window_size = 32  # Add window_size
...
def test_fused_softmax_topk_max_pooling_memory_and_speed():
    ...
    is_causal = True
    block_size = 64
    window_size = 64 # Add window size
    
    dtype = torch.bfloat16
    device = "cuda"
    
    print(f"Config: B={B}, L={L}, S={S}, h_kv={h_kv}, G={G} (h_q={h_q}), D={D}, topk={topk}, block_size={block_size}, window_size={window_size}")
    ...
    def run_fused():
        ...
        _, scores = online_softmax_topk_head(q_t, lmks_t, lse_swa_t, topk, block_size, window_size, is_causal)
        ...
        for _ in range(5):
            q_t.grad = None
            lmks_t.grad = None
            _ = online_softmax_topk_head(q_t, lmks_t, lse_swa_t, topk, block_size, window_size, is_causal)
            _, scores = online_softmax_topk_head(q_t, lmks_t, lse_swa_t, topk, block_size, window_size, is_causal)
            loss = (scores * grad_output).sum()
            loss.backward()
        torch.cuda.synchronize()
    ...
    def run_ref():
        ...
        def forward_only():
            _ = ref_softmax_topk_max_pooling(q_t, lmks_t, lse_swa_t, topk, block_size, window_size, is_causal)
        
        def forward_backward():
            _, scores = ref_softmax_topk_max_pooling(q_t, lmks_t, lse_swa_t, topk, block_size, window_size, is_causal)
            loss = (scores * grad_output).sum()
            loss.backward()
...
@pytest.mark.parametrize(
    "test_name, B, q_len, kv_len, h_kv, G, D, topk, block_size, window_size, mode, q_offset",
    [
        ("train_basic", 2, 1024, 1024, 2, 8, 128, 16, 64, 64, False, 0),
    ]
)
def test_train_inference_correctness(test_name, B, q_len, kv_len, h_kv, G, D, topk, block_size, window_size, mode, q_offset):
    """
    ...
    print(f"        mode={mode}, q_offset={q_offset}")
    print(f"{'='*70}")
    ...
    if mode == 'inference' and q_len < kv_len:
    ...
    # 输入隔离
    q_fused = q_raw.clone().detach().requires_grad_(mode == 'train')  # 推理模式不需要梯度
    lmks_fused = lmks_raw.clone().detach().requires_grad_(mode == 'train')
    
    q_ref = q_raw.clone().detach().requires_grad_(mode == 'train')
    lmks_ref = lmks_raw.clone().detach().requires_grad_(mode == 'train')
    ...
    indices_ref, scores_ref = ref_softmax_topk_max_pooling(
        q_ref, lmks_ref, lse_swa.float(), topk, block_size, window_size, is_causal, q_offset=q_offset
    )
    
    # Fused Kernel
    indices_fused, scores_fused = online_softmax_topk_head(
        q_fused, lmks_fused, lse_swa, topk, block_size, window_size, is_causal,
        mode=mode, q_offset=q_offset
    )
...
if __name__ == "__main__":
    ...
    test_cases = [# test_name, B, q_len, kv_len, h_kv, G, D, topk, block_size, window_size, mode, q_offset
        # 训练场景
                ("train_basic", 2, 1024, 1024, 2, 8, 128, 16, 64, 64, "train", 0),
                ("train_large_batch", 4, 2048, 2048, 2, 8, 128, 16, 64, 64, "train", 0),
                ("train_non_divisible", 2, 999, 999, 2, 8, 128, 16, 64, 64, "train", 0),
                
                # 推理场景
                ("prefill_chunk1", 1, 1024, 1024, 2, 8, 128, 16, 64, 64, "inference", 0),
                ("prefill_chunk2", 1, 1024, 2048, 2, 8, 128, 16, 64, 64, "inference", 1024),
                ("prefill_chunk3", 1, 1024, 3072, 2, 8, 128, 16, 64, 64, "inference", 2048),
                ("prefill_non_divisible", 1, 512, 1536, 2, 8, 128, 16, 64, 64, "inference", 1024),
                ("decode_step1", 1, 1, 1025, 2, 8, 128, 16, 64, 64, "inference", 1024),
                ("decode_step2", 1, 1, 2049, 2, 8, 128, 16, 64, 64, "inference", 2048),
                ("edge_small_topk", 1, 512, 1024, 2, 8, 128, 4, 64, 64, "inference", 512),
                ("edge_large_window", 1, 1024, 2048, 2, 8, 128, 16, 64, 128, "inference", 1024),
    ]
