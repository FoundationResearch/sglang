...
def hsa_torch_ref(q, k, v, weights, indices, *, chunk_size: int, sm_scale: float, block_q: int, mask_last_token: bool = False, window_size: int = -1):
    """
    Reference implementation (Group Dense version):
    - weights shape: (B, q_blocks, H, K), H is #KV heads, shared within a group
    - For each query block and selected K chunks:
      p = softmax_s(q @ k^T * sm_scale)
      o_k = p @ v
    - Final output: o = sum_k (weights[:, :, :, k] * o_k)

    Shape conventions:
    - q: (B, L, HQ, D)
    - k, v: (B, L, H, D)
    - weights: (B, q_blocks, H, K)
    - indices: (B, q_blocks, H, K) or (B, L, H, K)
    ...
    # [Group Dense] weights: (B, Bq, H, K) — group 内共享，需要 repeat 到 HQ
...
def build_block_indices_block_M(
    ...
    """
    Build block_indices tensor:
    - Within each window of block_M tokens, adjacent tokens (t, t+1) share overlap_ratio of selected blocks.
    - Selected indices are sorted in ascending order per query.
    - Padded with -1 if fewer than S blocks.

    Args:
        B: batch size
        SEQ_LEN: sequence length
        H: number of heads
        S: number of selected blocks per query
        block_size: size of each block
        overlap_ratio: overlap ratio between adjacent tokens [0,1]
        block_M: number of tokens per window (e.g. pair=2, for block_M kernel set to M)
...
def hierarchical_sparse_attention_block_M(batch, heads, q_len, kv_len, head_dim,
    ...
    # [Group Dense] weight_shape uses head_kv (group内共享)
    ...
    if block_M is None or block_M <= 0:
        block_M = 64 
    # block_M = 64
    print("Using block_M =", block_M, "for fwd_block_M kernel (Group-Dense)")
    ...
    def hsa_block_M(
        ...
        with T.Kernel(tilelang.cdiv(q_len, block_M), batch * heads, threads=num_threads) as (bx, bz):
            ...
            for k_blk_idx in T.Pipelined(pipeline_limit, num_stages=num_stages):
                ...
                # [Group Dense] W indexed by i_h_kv (group内共享权重)
...
def hierarchical_sparse_attention_bwd_dqkv_block_M(
    ...
    # [Group Dense] weight/dw shape uses heads_kv (group内共享)
    weight_shape = [batch, q_len, heads_kv, num_weights]
    dw_shape = [batch, q_len, heads_kv, num_weights]
    
    # [Modified] block_M is independent of groups
    if block_M is None or block_M <= 0:
        block_M = 64
    # block_M = 64
    print("Using block_M =", block_M, "for bwd_block_M kernel (Group-Dense)")
        ...
        with T.Kernel(NS_kv, B * heads, threads=num_threads) as (i_s, i_bh):
            ...
            for ip in T.Pipelined(ip_start, NP, num_stages=num_stages):
                ...
                if q_blk_idx > i_s:
                    ...
                    # [Step 1] 提前加载 W (Group Dense: 用 i_h_kv 索引)
                    ...
                    # [Group Dense] DW 用 i_h_kv 索引，同一 group 内多个 head 需要 atomic_add
                    for dw_i in T.Parallel(M):
                        T.atomic_add(DW[i_b, base_t + dw_i, i_h_kv, i_s], dw_row_sum_shared[dw_i])
...
class _hsa_block_M_attention_dense(torch.autograd.Function):
    ...
    def forward(ctx, Q, K, V, W, block_size, sm_scale, block_M, mask_last_token , window_size,
        ...
        # Q: [B, L, HQ, D]
        # K, V: [B, L, H, D]
        # W: [B, L, H, S] (Group Dense: group内共享权重)
    ...
    def backward(ctx, grad_output):
        ...
        # [Group Dense] dw_shape uses H (kv heads), not HQ
        dw_shape = [B, SEQ_LEN, H, num_weights]
        ...
        # [Group Dense] DW uses atomic_add (multiple query heads in a group write to same kv head), must be float32 and zeroed
        DW = torch.zeros(dw_shape, dtype=torch.float32, device=Q.device)
        ...
        # [Group Dense] DW: float32 -> W.dtype
        DW = DW.to(W.dtype)
...
def HSA_block_M_group_dense(Q, K, V, W, block_size=64, sm_scale=None, block_M=64, mask_last_token=True, 
                           window_size=-1,
...
def HSA_dense_interface(
    ...
    q_group_sum = score_q.view(B, L, H, G, D).sum(dim=3)  # (B, L, H, D)
    scores_h = torch.einsum("blhd,bshd->blhs", q_group_sum, lmk)  # (B, L, H, S)
    scores_h = scores_h * (1.0 / math.sqrt(D))
    allow = _create_chunk_window_mask(L, S, block_size, window_size, q.device)
    scores_h = scores_h.masked_fill(~allow.view(1, L, 1, S), float("-inf"))

    lse_last = lse_swa.unsqueeze(-1)  # (B, L, H, 1)
    if not enable_softmax1:
        cat_scores = torch.cat([scores_h, lse_last], dim=-1)
        lse_idx = -1
    else:
        ones = torch.zeros((B, L, H, 1), device=q.device)
        cat_scores = torch.cat([scores_h, lse_last, ones], dim=-1)
        lse_idx = -2
    chunk_weights_all = torch.softmax(cat_scores, dim=-1)  # (B, L, H, S+...)

    out = HSA_block_M_group_dense(
        Q=q,
        K=k,
        V=v,
...
def HSA_dense_interface_ref(
    ...
    if not enable_softmax1:
        ...
        ones = torch.zeros((B, L, H, 1), device=q.device, dtype=scores.dtype)
        cat_scores = torch.cat([scores, lse_last, ones], dim=-1)
        lse_idx=-2
    chunk_weights_all = F.softmax(cat_scores, dim=-1)  # (B, L, H, S+...)

    num_kv_blocks = S  
    block_indices_dense = torch.arange(num_kv_blocks, dtype=torch.int32, device=q.device)
    block_indices_dense = block_indices_dense.view(1, 1, 1, num_kv_blocks).expand(B, L, H, num_kv_blocks)

    W_for_ref = chunk_weights_all[:, :, :, :S]  # (B, L, H, S)
    ...
    out = hsa_torch_ref(
        q_in.float(), k_in.float(), v.float(), W_for_ref.float(), block_indices_dense,
...
def test_dense_interface_vs_ref(
    ...
    # 模拟 lhsa_layer.py：先生成 (B, L, HQ) 再 logsumexp 聚合到 (B, L, H)
    lse_swa_raw = torch.randn((B, L, HQ), device=device, dtype=torch.bfloat16)
    G = HQ // H
    lse_swa = lse_swa_raw.view(B, L, H, G).logsumexp(dim=-1).contiguous()  # (B, L, H)
    ...
    # [核心验证] scores 等价性: einsum weights vs topk kernel weights
    # chunk_weights_all 的前 S 列对应 HSA chunk 权重
    S = num_chunks
    check("Weights(einsum vs topk)", w_hsa[:, :, :, :S], w_ref[:, :, :, :S], thr=5e-3)

    # 前向输出对比: 包含 scores 差异 + tilelang kernel vs torch_ref 差异
    ...
    # backward: 由于 hsa_torch_ref(PyTorch) vs tilelang kernel 的 backward 路径不同，
    # 加上 topk kernel backward vs einsum backward 的差异，阈值适当放宽
...
def main_block_M_correctness():
    """
    Verify forward and backward numerical correctness of HSA_pair wrapper
    against hsa_torch_ref (Dense Mode)
    """
    import math
    import torch
    import torch.nn.functional as F
    ...
    # [Group Dense] 生成权重: [B, SEQ_LEN, H, num_kv_blocks] (group内共享)
    ...
    # HSA_pair 前向 (Dense)
    O_hsa = HSA_block_M_group_dense(Q, K, V, W, block_size=block_size, sm_scale=scale, block_M=block_M, mask_last_token=mask_last_token, window_size=window_size)
...
def main_block_M_latency():
    """
    Benchmark tilelang HSA_block_M (Dense) FWD, BWD, and combined (FWD+BWD) latency
    """
    import torch
    import torch.nn.functional as F
    import time
    ...
    for _ in range(num_warmup):
        O = HSA_block_M_group_dense(Q_tile, K, V, W, block_size=block_size, sm_scale=scale, block_M=block_M, mask_last_token=mask_last_token, window_size=window_size)
...
def test_correctness_fp32(B, SEQ_LEN, H, HQ, D, block_size, window_size):
    ...
    # [Group Dense] 权重形状为 (B, SEQ_LEN, H, num_kv_blocks)
...
def benchmark_dense_interface_weight_methods(
    ...
    # 模拟 lhsa_layer.py：先生成 (B, L, HQ) 再 logsumexp 聚合到 (B, L, H)
    lse_swa_raw = torch.randn((B, L, HQ), device=device, dtype=dtype)
    G = HQ // H
    lse_swa = lse_swa_raw.view(B, L, H, G).logsumexp(dim=-1).contiguous()  # (B, L, H)
    ...
    print(f"[HSA_dense_interface (einsum + tilelang)]    FWD {a_fwd:.3f} ms | BWD {a_bwd:.3f} ms | Total {a_total:.3f} ms")
    print(f"[HSA_dense_interface_ref (topk + torch_ref)] FWD {b_fwd:.3f} ms | BWD {b_bwd:.3f} ms | Total {b_total:.3f} ms")
