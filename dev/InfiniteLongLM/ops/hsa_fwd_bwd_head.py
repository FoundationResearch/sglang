...
def hsa_torch_ref(q, k, v, weights, indices, *, chunk_size: int, sm_scale: float, block_q: int, mask_last_token: bool = True):
    ...
    - 支持 q_len != kv_len 的情况（chunk prefill）
    
    形状约定:
    - q: (B, q_len, HQ, D)        # Q 可以比 KV 短
    - k, v: (B, kv_len, H, D)     # KV 是完整序列
    - weights: (B, q_blocks, HQ, K)
    - indices: (B, q_blocks, H, K) - indices 指向 KV 中的 chunk 索引
    - 返回: o_ref: (B, q_len, HQ, D) float32
    """
...
def hierarchical_sparse_attention_block_M(batch, heads,  head_dim, q_len=None, kv_len=None,
    ...
    if use_cache:
        ...
        q_len_var = q_len
        kv_len_var = kv_len
    q_shape = [batch, q_len_var, heads, head_dim]
    kv_shape = [batch, kv_len_var, head_kv, head_dim]
    weight_shape = [batch, q_len_var, heads, num_weights]
    block_indices_shape = [batch, q_len_var, head_kv, selected_blocks]
    ...
    def hsa_block_M(
        ...
        with T.Kernel(tilelang.cdiv(q_len_var, M), NV, batch * head_kv, threads=num_threads) as (bx, by, bz):
            ...
            if T.get_thread_binding() == 0:
                ...
                for _ in T.serial(S * M):
                    ...
                    for q_idx in T.serial(M):
                        tq = base_t + q_idx
                        if tq < q_len_var and pointers[q_idx] < valid_lens[q_idx]:
                                has_valid = 1
...
class _hsa_block_M_attention(torch.autograd.Function):
    ...
    def forward(
        ...
        if use_cache:
            assert B == 1, f"Inference mode (use_cache=True) only supports batch=1, got batch={B}"
        ...
        fwd_kernel = hierarchical_sparse_attention_block_M(
            batch=B,
            heads=HQ,
            q_len=None if use_cache else L,
            kv_len=None if use_cache else L_kv,
            head_dim=D,
    ...
    def backward(ctx, do):
        ...
        if ctx.use_cache:
            raise RuntimeError("Backward pass is not supported in inference mode (use_cache=True)")
...
def HSA_block_M_head(
    ...
    cos=None, sin=None, mode='train',
):
    """
    HSA Block M Head Attention
    Support train and inference mode.
...
def main_block_M_correctness():
    ...
    # HSA_pair 前向
    O_hsa = HSA_block_M_head(Q, K, V, W, block_indices, block_size=block_size, sm_scale=scale, block_M=block_M, mask_last_token=mask_last_token,mode=mode)
    ...
    O_hsa_bwd = HSA_block_M_head(Q, K, V, W, block_indices, 
                         block_size=block_size, sm_scale=scale, block_M=block_M, mask_last_token=mask_last_token,mode=mode)
...
def main_block_M_latency():
    ...
    for _ in range(num_warmup):
        O = HSA_block_M_head(Q_tile, K, V, W, block_indices, block_size=block_size, sm_scale=scale, block_M=block_M, mask_last_token=mask_last_token, mode=mode)
...
def main_rope_correctness():
    ...
    o_hsa = HSA_block_M_head(
        ...
        mask_last_token=True, enable_inverse_rope=True, cos=cos, sin=sin, mode=mode
    )
...
def main_rope_latency_memory():
    ...
    for _ in range(num_warmup):
        O = HSA_block_M_head(Q, K, V, W, block_indices, 
                            ...
                            enable_inverse_rope=False, mode=mode)
    ...
    for _ in range(num_warmup):
        O = HSA_block_M_head(Q_rope, K_rope, V, W, block_indices, 
                            block_size=block_size, sm_scale=scale, 
                            block_M=block_M, mask_last_token=mask_last_token,
                            enable_inverse_rope=True, cos=cos, sin=sin, mode=mode)
        O.backward(grad_output)
...
def test_correctness_fp32(B, SEQ_LEN, H, HQ, D, S, block_size):
    ...
    # 3. 前向计算
    O_hsa = HSA_block_M_head(Q, K, V, W, block_indices, block_size, scale, block_M, mask_last_token, "float", "float", 64, mode=mode)
...
@pytest.mark.parametrize("B, q_len, kv_len, H, HQ, D, S, block_size, mode, mask_last_token", [
    (1, 1024, 1024, 1, 8, 128, 4, 64, 'train', True),      # 基础训练场景
])
def test_train_inference_correctness(B, q_len, kv_len, H, HQ, D, S, block_size, mode, mask_last_token):
    """
    检验 HSA_block_M_head 封装类的前向传播数值正确性
    与 hsa_torch_ref 进行对比
    
    支持以下场景:
    - 训练: q_len == kv_len, use_cache=False, batch 可以大于 1
    - 推理 chunk prefill: q_len <= kv_len, use_cache=True, batch 必须为 1
    - 推理 decode: q_len=1, use_cache=True, batch 必须为 1
    """
    device = "cuda"
    dtype = torch.bfloat16
    block_M = 4
    ...
    print(f"\n{'='*70}")
    print(f"Test Config: B={B}, q_len={q_len}, kv_len={kv_len}, H={H}, HQ={HQ}, D={D}, G={G}, S={S}, block_size={block_size}")
    print(f"mode={mode}, mask_last_token={mask_last_token}")
    print(f"{'='*70}")
    ...
    print(f"[Tilelang HSA_block_M_head] vs [Torch Reference]:")
    print(f"前向最大误差: {fwd_max_diff:.6e}, 平均误差: {fwd_mean_diff:.6e}")
    
    # 断言前向误差在可接受范围内
    assert fwd_max_diff < 0.01, f"前向误差过大: {fwd_max_diff:.6e}"
    
    print(f"✅ Test PASSED: B={B}, q_len={q_len}, kv_len={kv_len}, mode={mode}")
