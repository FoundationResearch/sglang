...
from data.data_transform import process_numpy_example
import models
...
def main():
    ...
    elif args.data.data_type == "numpy":
        transform = process_numpy_example
    ...
    print(f'inv freq before build parallel: {model.model.rotary_emb.inv_freq}')
    ...
    print(f'inv freq after build parallel: {model.model.rotary_emb.inv_freq}')
    ...
    if args.train.global_rank == 0:
        ...
        model_assets = [model_config, tokenizer if args.data.data_type in ["plaintext", "numpy"] else chat_template]
