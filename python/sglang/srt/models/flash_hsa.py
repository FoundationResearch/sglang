...
# enable TF32 tensor cores for the fp32 chunk_attn_pool einsum. The
...
# vectorised chunk_attn_pool (ported from friend's
...
def _chunk_attn_pool_impl(
    ...
    # cast to fp32 inside the compiled fn so inductor can fuse the cast
...
def _get_flashhsa_padded_vocab_size(config) -> int:
    ...
    # (Item #1): when `enable_external_lmk_embed=True` the LMK token
...
class FlashHSAInnerXHierarchicalSparseAttention(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # (Item #3): track lmk_q_lora_dim so backend can detect production
        ...
        # layerwise_lmkq_norm — when True the landmark query is RMSNorm'd
        ...
        # Writer activation must follow enable_prior_query, NOT
    ...
    def forward(
        ...
        lmk_q_fused = None
        # raw HSA q (pre-norm, pre-RoPE) reserved for lmk_q LoRA residual.
        ...
        # --- Selection query ---
        if self.enable_lmk_q_proj and self.lmk_q_proj is not None:
            # Separate lmk_q_proj for page selection. applies RoPE below
            ...
            if self.lmk_q_full_dim:
                ...
                if self.layerwise_lmkq_norm:
                    # RMSNorm over the FULL projected width (norm
        ...
        # apply RoPE to lmk_q (selection query) when apply_hsa_rope=True
        ...
        if (
            ...
            # rotary_emb takes a key tensor it RoPE's alongside the query;
    ...
    def _maybe_write_chunk_lmk_k(self, forward_batch, sel_q, hsa_k) -> None:
        ...
        # cache (req_idx, prefix_len, extend_len) once per forward to
        ...
        # vectorise across all newly-complete chunks. Replaces the
        ...
        device = sel_q.device
        # gather indices are CONTIGUOUS in the common case — first_kept
        ...
        # vectorised chunk_attn_pool (no K repeat_interleave, lse-E[z]
        # entropy, optional torch.compile). Keeps logits at (N, S, h_kv, G)
        # instead of (N, S, h_q) — 8x less memory bandwidth for K read.
        # pass bf16 in — the compiled impl now does the fp32 cast inside
        ...
        # pre-allocate slots for future decode chunks so the decode-time
    ...
    def _maybe_write_decode_chunk_lmk_k(
        ...
        """: decode-time chunk_attn_pool write.

        Two paths:
          * Non-CG (fast eager): cache `is_lmk_step` per forward via ,
            early-return on the 63/64 non-boundary steps. Cheap.
          * CG capture: no host-side syncs allowed; run the compute
        ...
        # Fast eager path: cache + early-return on non-boundary steps.
        # Skipped during CG capture (Python-side sync is illegal there) —
        # we fall through to the device-side path below.
        is_capturing = (
        ...
        if not is_capturing:
            ...
            if cache is None:
                ...
                if input_ids is None or lmk_id_py < 0:
                    ...
                    return
                # Single sync for the whole forward (was 16x earlier).
...
class FlashHSAInnerXModel(nn.Module):
    ...
    def __init__(self, config, quant_config=None, prefix: str = "") -> None:
        ...
        # (Item #1): standalone landmark embedding when
...
class HSAForCausalLM(nn.Module):
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        # (Item #3): production checkpoints with `lmk_q_lora_dim > 0`
        ...
        # fused QKV path ( speed) and matches the alignment loader in
        ...
        for name, loaded_weight in weights:
            ...
            # TP-shard the attention norm weights (q_norm / k_norm / lmk_q_norm).
            # For the SWA layer's OLMo3-style QK norm and the HSA layer's
            # lmk_q_norm, under TP the param dim = total / tp_size, so we slice
            # the current rank's shard out of the checkpoint's full weight. The
            # HSA layer's per-head q_norm/k_norm have dim head_dim, unaffected by
            # TP — the param.shape != loaded_weight.shape check skips it.
            if (
                name in params_dict
