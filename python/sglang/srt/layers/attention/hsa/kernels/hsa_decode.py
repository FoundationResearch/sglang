...
def hsa_decode_paged_fwd_kernel(
    ...
    # optional SWA blend inputs (pointers; None at Python side when blend disabled)
    ...
    # fused chunk-weight + page-aggregate path.  Eliminates the
    ...
    # split-K parallelism — grid is (B, HQ, SPLIT_K) instead of (B, HQ).
    ...
    # page iteration range for this split.
    ...
    if SPLIT_K == 1:
        ...
        # split-K path — write partial fp32 output to per-split slot.
...
# cached split-K config — picked once per process.  GB200 has 144 SMs;
...
def hsa_decode_paged_fwd(
    *,
    ...
    # optional SWA blend in the same kernel
    swa_o_inner: Optional[torch.Tensor] = None,   # [B, HQ, D] fp32
    swa_w_q: Optional[torch.Tensor] = None,       # [B, HQ]    fp32
    # in-kernel chunk-weight path.  When provided, the kernel skips the
    ...
    # split-K parallelism — defaults to autotuned based on B*HQ vs num_sms.
    ...
    # accept int32 OR int64 page_ids; kernel does `.to(tl.int32)` in-register
    ...
    # split-K decision.  Decode is the hot path (N small, HQ ≤ ~32 for
    ...
    if split_k > 1:
        # reduction kernel — sums per-split partials + applies BLEND_SWA + bf16 cast.
