...
motivation: at L=16K the SWA branch dominated the profile
...
This kernel mirrors the sparse G-fusion trick: process all G
...
Mask logic identical to unified-kernel path:
  - causal: q_pos >= k_pos
...
def hsa_swa_extend_fwd(
    *,
    ...
    page_size: int,
    sm_scale: float,
    # default block_m=8 num_warps=2 — Blackwell sweep showed 1.27x faster
    ...
    # kernel casts loaded slot to int64 inline (line ~160), so int32
    ...
    hsa_swa_extend_kernel[grid](
        ...
        BLOCK_M=int(block_m),
        BLOCK_N=int(block_n),
        # drop num_warps 4 -> 2 (kernel is memory-bound at the tuned
