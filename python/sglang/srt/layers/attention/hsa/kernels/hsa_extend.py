...
BLOCK_M=4 with block-diagonal masking — packs adjacent Q tokens for one
TC matmul per page, wastes 3/4 of the QK FLOPs but TC throughput beats CUDA
cores by enough to win net 1.3x at L=16K.

Also pack all G q-heads of a kv-head into the same program. Since
...
def hsa_extend_paged_fwd_kernel(
    ...
    # optional epilogue — fuse SWA blend
...
def hsa_extend_paged_fwd(
    *,
    ...
    num_warps: int = 2,
    num_stages: int = 2,
    # optional fused SWA blend epilogue.
