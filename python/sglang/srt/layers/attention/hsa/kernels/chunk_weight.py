...
Python/dispatch overhead in `forward_decode` after .
"""
...
def fused_chunk_weight_per_qhead_kernel(
    ...
    # accept bf16 OR fp32 scores. Cast to fp32 for numerical stability.
    if SCORES_IS_BF16:
        scores = tl.load(
...
def fused_chunk_weight_h_kv_kernel(
    scores_ptr,           # [B, H, K]  bf16 OR float32 (accepts bf16)
    lse_hq_ptr,           # [B, HQ]    bf16 OR float32 (per-q-head LSE, )
    selected_pages_ptr,   # [B, H, K]  int32
    w_q_ptr,              # [B, HQ, K] bf16  (output, broadcast across Gh)
    swa_w_kv_ptr,         # [B, HQ]    float32 (output, HQ granularity)
    H: tl.constexpr,
    HQ: tl.constexpr,
    Gh: tl.constexpr,
    K: tl.constexpr,
    ...
    """: fused legacy h_kv chunk-weight softmax + GQA broadcast +
    inlined per-q-head logsumexp aggregation.

    previously took `lse_kv [B, H]` (computed via torch.logsumexp
    ...
    # load per-q-head LSE for this h_kv group and reduce inline.
...
def fused_chunk_weight_h_kv_decode(
    ...
    lse_hq: torch.Tensor,            # [B, HQ_hsa]   bf16 OR float32
    ...
    now takes per-q-head LSE [B, HQ_hsa] (saves the upstream
    ...
    # accept int32 OR int64 page_ids — kernel's `sel_pages >= 0` works
    ...
    # output swa_w at HQ granularity directly (saves broadcast op).
...
def fused_chunk_weight_per_qhead_decode(
    ...
    """
    # accept bf16 OR fp32 scores. fp32 was forced before, but the
