...
Folds the per-layer `cand_page_ids` / `cand_mask` materialisation
...
the reference: switching either inside the kernel regressed perf (q.sum unroll
...
The downstream `torch.topk(scores, K, sorted=False)` consumes the
...
def fused_selector_score_kernel(
    q_ptr,                 # [B*HQ*D]             bf16 — raw q (sums over G inline)
    ...
    effective_cands is computed inline from cache_seqlens.
    q.sum over GQA group computed inline via 2D load — saves the host-side
    ...
    # 2D load [G, D] of q for this (b, h_sel) group, then sum over G.
    ...
    # Compute effective_cands inline from cache_seqlens.
...
def fused_selector_score_decode(
    *,
    q: torch.Tensor,                # [B, HQ, D] bf16 — raw q BEFORE GQA sum
    H_sel: int,                     # number of HSA kv-heads (= HQ // G)
    cache_seqlens: torch.Tensor,    # [B] int32 — per-request KV length
    ...
    sm_scale: float,
    page_size: int,
    hsa_window: int = 0,            # window-exclusion computed in kernel
):
