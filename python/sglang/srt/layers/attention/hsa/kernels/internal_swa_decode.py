...
Replaces the  chain (~15 ops per layer per decode step):
...
optional split-W parallelism — grid becomes (B*HQ_hsa, SPLIT_W) so we
...
def fused_internal_swa_decode_kernel(
    ...
    # each split processes its slice of NUM_W_BLOCKS.
...
def fused_internal_swa_decode(
    *,
    ...
    # kernel always writes every (b, hq) slot (lines 132-133), so torch.empty
