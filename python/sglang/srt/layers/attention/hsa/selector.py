...
# Try to import the fused online topk kernel (preferred path); fall back to torch topk on failure
...
def select_topk_pages_decode_fused(
    *,
    ...
    """Top-k page selection via the fused online_topk_group kernel.

    Matches the select_topk_pages_decode interface; returns HSASelectionResult.
    Returns None if the fused kernel is unavailable (caller should fall back to the torch path).

    Args:
      q: [B, HQ*D] or [B, HQ, D]  — query tensor
      cand_page_ids: [B, C] int32 padded with -1
      cand_mask: [B, C] bool
      cand_repr: [B, C, H_sel, D] — landmark representation of candidate chunks
      topk: number of top-k entries to select
      page_size: chunk/page size
      sm_scale: optional attention scale (handled inside the fused kernel)
      selection_strategy: "group" or "head" (fused kernel supports group mode only)
    ...
    if q.dim() == 2:
        ...
        return None  # unsupported shape, fall back
    ...
    if G is not None and G >= 1:
        # NOTE on experiment: tried calling _online_topk_head_maxpool here
        ...
        # drop redundant ops. Key invariants for the hot path (large C):
        ...
        # fuse the 6-op score chain (2 casts + einsum + prior add + max-over-G
        ...
        # entropy bias (prior_b) is added per q-head inside the kernel.
        ...
        # keep the topk VALUES (== gather(scores_kv_sel, topk_idx_kv)) instead
    ...
    # decode fast path — when q has a single query token (q_4d.shape[1]==1),
    ...
    if q_4d.shape[1] == 1 and G is None:  # G>1 path was already handled above
        ...
        # skip the trailing sort + gather (selected pages feed into
        # softmax-aggregated downstream kernels — permutation-invariant).
        # sorted=False — sbtopk skips its internal bitonic value-sort.
    ...
    # Map the chunk index back to page_id (candidate set is [0, C_max), index == page_id)
