...
class HSAAttnBackend(AttentionBackend):
    ...
    def __init__(self, model_runner: ModelRunner, **kwargs):
        ...
        # align fallback with FlashHSAConfig default (16, see
        ...
        # stale value. This mismatch was caused by exactly this kind of drift —
        ...
        # default to False (maxpool fast path) when cfg doesn't specify.
        ...
        # Env override has highest priority so consistency
        ...
        # denominator. align fallback with FlashHSAConfig default
        ...
        # cuda-graph persistent buffers (allocated in init_cuda_graph_state).
        ...
        # auto-init per-q-head lmk_k pools when the config asks for the
        ...
        # disabling all of slot reuse, prior_b, and decode
        ...
        # Surface enable_prior_query so the extend pure-PyTorch
        ...
        # earlier silent disable state on demand. Set to "1" to skip auto-init
        # and force pools to stay None (matches earlier production behaviour).
    ...
    # shared scalar cache for prefill hot paths (selection + internal SWA +
    ...
    def _run_selection_decode(
        ...
        *,
        ...
        # step in init_forward_metadata.  No per-layer clone needed.
        ...
        # effective_cands (= clamp(min(seq_lens // ps, (seq_lens - hsa_window) // ps)))
        ...
        # avoid `effective_cands.max().item()` sync.  Use the metadata-known
        ...
        # Use >= (not >) so a G=1 MHA prior-query checkpoint
        ...
        # Legacy h_kv fast path — fused_selector_score_kernel internalises
        # the whole arange/expand/lt/masked_fill/sum chain plus gather+Q·K+mask
        # in one launch.  Followed by torch.topk (, sorted=False).
        ...
        if not per_qhead_active and not unified:
            ...
            # pass raw q (not GQA-summed) — kernel does the sum over G inline.
            ...
            # so selected_page_ids equals top_idx_d directly.  drop the
            ...
            # keep page_ids as int64 — both chunk_weight and hsa_decode
            # kernels now accept either dtype (load is in-register cast).
            md.hsa_selected_page_ids = top_idx_d
            # keep selected_scores in bf16 — chunk_weight kernel casts inline.
        ...
        # effective_cands [B] tensor.  cache layer-invariant scratch in
        ...
        if per_qhead_active:
            # slots = req_to_chunk_pool.gather_slots(req_pool_indices,
            ...
            # batch the lmk_k + prior_b gathers across ALL layers in one
        ...
        # for the per-qhead path, hand the kernel q_offset+hsa_window so
    ...
    def _compute_internal_swa_decode(
        ...
        *,
        ...
        lse_hq : [B, HQ_hsa] bf16  (per-q-head logsumexp; reduce to lse_kv
                                    in the consuming chunk_weight kernel via )
        """
        B, _, D = q_hsa.shape
        device = q_hsa.device
        ...
        # fused streaming-flash-attention triton kernel.  Collapses ~15
        # ops ( chain: window construction, K/V gather, two einsums,
        ...
        # drop the torch.logsumexp(lse_hq → lse_kv) — the legacy h_kv
    ...
    def init_forward_metadata(self, forward_batch: ForwardBatch):
        ...
        # cuda-graph buffer path.  When persistent buffers are allocated
        ...
        # pointers stay valid across replays.  already makes max_seqlen_k
    ...
    def init_forward_metadata_capture_cuda_graph(
        ...
        # .2: capture's run_once calls model.forward directly (NOT
    ...
    def forward_decode(
        ...
        **kwargs,
        ...
        if split_info is not None:
            ...
            # Internal SWA on HSA heads (LHSA semantics).
            # returns per-q-head lse_hq directly (kernel reduces over Gh inline).
            ...
            # `valid = selected_page_ids >= 0` was used only in the
            ...
            if per_qhead_fusion and hsa_window > 0:
                # fused chunk-weight softmax (8 ops -> 1 triton kernel).
                ...
                # chunk-weight is folded into hsa_decode_paged_fwd's
            ...
            # fuse the SWA blend into hsa_decode_paged_fwd's epilogue.
            # Need swa_w_q at [B, HQ_hsa] granularity.  Both per_qhead and
            # legacy h_kv paths now ALWAYS output swa_w_q already
            ...
            # when on the h_kv path (per_qhead_scores is None) AND
            ...
            # HSA-345M has HQ_swa == 0 — skip the empty+slice-write that
    ...
    def _compute_internal_swa_extend_batched(
        ...
        *,
        ...
        single-batch fused mask path. The triton extend kernel now natively
        ...
        # placeholder per-q-head LSE for empty/no-window early returns.
        ...
        # shared per-prefill scalar cache (was 2 .item() per layer here).
        ...
        # keep page_table_1's int32 — the kernel internally casts each
        ...
        # dedicated G-fused HSA-SWA kernel — packs all GQA q-heads per
        ...
        # keep swa_o in bf16 — the fusion at forward_extend step 7 mixes
        ...
        # stash per-q-head LSE for the per-q-head fusion path. Keep in
        ...
        # skip the per-layer torch.logsumexp(lse_raw, dim=Gh) — only the
    ...
    def _run_selection_extend(
        ...
        *,
        ...
        """Run selection for the extend/prefill phase (batched production path)."""
    ...
    def _run_selection_extend_batched(
        ...
        *,
        ...
        Reuses the training-side online_topk_group kernel call pattern:
        gather the shared LMK [1, S, H, D] once, then call
        online_topk_group(B=1, L=T, is_causal=True, q_offset=prefix_len);
        the kernel handles the causal mask + head-group sum internally.
        Memory drops from O(T × C_max × H × D) to O(S × H × D).
        Only batch=1 is supported (guaranteed by SGLang continuous batching).
        """
        md = self.forward_metadata
        if md is None:
            return
        ...
        # --- batch=1 only; fail loudly on multi-sequence to surface issues ---
        extend_seq_lens = md.extend_seq_lens
        extend_prefix_lens = md.extend_prefix_lens
        assert extend_seq_lens is not None and extend_prefix_lens is not None, \
        ...
        # --- compute prefix_len and total sequence length ---
        # shared per-prefill scalar cache (was 2 .item() + 1 req_idx .item() per layer).
        ...
        S = total_seq_len // page_size  # total number of LMK chunks
        ...
        # already in the pool.
        # Use >= (not >) — G=1 MHA prior-query (hq_hsa == h_hsa) also
        ...
        if per_qhead_ext_active:
            # cache (S, slots_i64) once per prefill — slots only depends
            ...
            if slots_cache is None or slots_cache[0] != S:
                req_idx = _req_idx_cached  # cached
            ...
            # --- 1. gather the shared LMK [S, H, D] once ---
        ...
        # --- 2. unified_retrieval dim reduction (matches training) ---
        unified = split_info is not None and split_info.get("unified_retrieval", False)
        retrieval_dim = split_info.get("retrieval_dim", None) if split_info is not None else None
        if unified and retrieval_dim is not None:
            tp_size = get_attention_tp_size()
            ...
            # matches training: rearrange('S H D -> S 1 (H D)')
        ...
        # --- 3. call online_topk_group (matches training) ---
        # The G=1/prior-query per-q-head exact path (below) runs in
        ...
        if not _on_per_qhead_path:
            assert _online_topk_group is not None, (
                "Fused online_topk_group kernel not loaded; cannot run extend selection. "
                "Ensure hsa-kernel-main/ops/topk_group.py imports correctly."
            )

        # Q: [T, HQ_sel, D] → [1, T, HQ_sel, D]
        # for unified_retrieval, HQ_sel=1, D=retrieval_dim
        q_4d = q_sel_3.unsqueeze(0)  # [1, T, HQ_sel, D_sel]
        lmk_4d = lmk_keys.unsqueeze(0)  # [1, S, H_sel_kernel, D_kernel]

        # q_offset = prefix_len, the global-sequence position of the first extend token
        if self._per_qhead_G_ext is not None and self._per_qhead_lmk_keys_full is not None:
            ...
            # The maxpool tilelang kernel tiles with
            ...
            if use_maxpool:
                ...
                # keep scores in their native dtype (bf16 from maxpool).
                scores_hq_sel_t = scores_hq_mp.squeeze(0)  # [T, h_q, K] bf16
                # skip the per-layer max-over-G that built `fused_scores`.
                ...
                # Re-add prior_b (entropy bias from chunk_attn_pool),
        ...
        # the kernel returns a chunk index (0-based), i.e. the page_id
        # invalid entries (-1) are left unchanged
        selected_page_ids = selected_page_ids.masked_fill(selected_page_ids < 0, -1)

        # if unified_retrieval, h_shared=1, so repeat to H_sel (original kv head count)
        if unified and retrieval_dim is not None:
    ...
    def _hsa_sparse_attn_extend(
        ...
        *,
        ...
        """Sparse HSA attention for the extend phase (Triton kernel)."""
        # Q-batched extend kernel (collapses (T, HQ) grid to (T/BLOCK_M, HQ)).
        md = self.forward_metadata
        import os as _os
...
.  Without it the chunk weights diverge from official — argmax
    ...
    def forward_extend(
        ...
        **kwargs,
        ...
        # HSA-345M has HQ_swa==0 — skip the dense_out_3 alloc + slice-write
        # of out_hsa into it, mirroring 's decode fast path. Per layer saves
        ...
        # second return is now `lse_raw` [T, HQ_hsa] (per-q-head) — the
        ...
        if per_qhead_fusion and hsa_window > 0:
            # reuse the existing fused_chunk_weight_per_qhead_decode kernel
            ...
            # compute lse_kv on demand here (per_qhead branch above
        ...
        # Step 6+7 fuse the SWA blend epilogue into the HSA sparse kernel.
        ...
        # kernel internally casts loaded swa_w_q to fp32 (see hsa_extend.py
        ...
        # when HQ_swa==0, out_hsa already covers all heads — return
