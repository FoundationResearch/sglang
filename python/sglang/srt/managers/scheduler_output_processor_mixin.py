...
class SchedulerOutputProcessorMixin:
    ...
    def _get_hsa_lmk_mask(self, req: Req, num_logprobs: int) -> list:
        """Build a mask over HSA LMK positions to filter LMK values out of input logprobs.

        Returns a bool list of length num_logprobs; True means the position is a
        real token (keep), False means it is an LMK token (drop).

        LMK positions in fill_ids follow a fixed pattern: the last token of every
        page_size tokens is an LMK, i.e. fill_ids[k] == lmk_id iff
        (k+1) % page_size == 0. Logprobs start at logprob_start_len in fill_ids,
        so an offset is applied.
        """
        if not getattr(req, "hsa_lmk_enabled", False):
            return [True] * num_logprobs
        lmk_id = req.hsa_lmk_id
        # The fill_ids range the logprobs cover starts at logprob_start_len, but
        # that index is relative to the LMK-inclusive sequence. In practice
        # input_token_logprobs corresponds to the last num_logprobs tokens of
        # fill_ids (when logprob_start_len=0 it covers all of fill_ids).
    ...
    def _process_input_token_logprobs(
        ...
        # HSA LMK filtering: input_token_logprobs includes logprobs at LMK
        # positions, but origin_input_ids has no LMKs — drop the LMK positions so
        # the two line up.
        if getattr(req, "hsa_lmk_enabled", False):
    ...
    def _process_input_top_logprobs(self, req: Req) -> None:
        ...
        # HSA LMK filtering: top logprobs also carry data at LMK positions — drop them.
        if getattr(req, "hsa_lmk_enabled", False):
            # The top-logprobs length matches input_token_logprobs_val before its
            # own filtering. By this point input_top_logprobs_val is already in
            # [None] + data[:-1] form, so LMK positions are identified via fill_ids.
            # Note: the first element is None (the first token has no predecessor)
            # and the last has been popped, so the covered fill_ids range matches
            # input_token_logprobs_val.
            total_with_lmk = len(req.input_top_logprobs_val)
            if total_with_lmk > 0:
                fill_ids = req.fill_ids
                # input_top_logprobs_val[0] = None (first token); val[i] corresponds
                # to the i-th token from logprob_start_len in fill_ids. Because of
                # the [None] + data[:-1] shift, val[i] is the logprob at
                # fill_ids[logprob_start_offset + i], where logprob_start_offset is
                # where logprobs begin in fill_ids. Simplest: check the token id at
                # the corresponding fill_ids position directly.
    ...
    def _process_input_token_ids_logprobs(self, req: Req) -> None:
        ...
        # HSA LMK filtering: token_ids logprobs also carry data at LMK positions — drop them.
        if getattr(req, "hsa_lmk_enabled", False):
